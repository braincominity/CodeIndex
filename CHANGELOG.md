...
#### Fixed
- **Extensionless shebang scripts are now indexed under their inferred language (#158)** — `FileIndexer` no longer limits indexability to known extensions and exact filename matches. When those checks miss, it now reads the first line and recognizes common shebang interpreters so extensionless scripts under directories like `libexec/` still enter the index as `shell`, `python`, `ruby`, `javascript`, `php`, `lua`, or `powershell`. The fallback stays limited to truly extensionless files, full-scan/update-mode refreshes now purge previously indexed rows when those files lose their shebang and become non-indexable, and Unix special files (for example FIFOs named `tool`, `tool.sh`, or `Dockerfile`) are skipped before any read/probe path can block indexing. Added regression coverage for language detection, directory scanning, full-scan/update-mode purge behavior, FIFO handling, and `BuildRecord`, and updated user/developer docs to document the shebang fallback plus the full PowerShell extension set. Affected: `src/CodeIndex/Indexer/FileIndexer.cs`, `src/CodeIndex/Cli/IndexCommandRunner.cs`, `src/CodeIndex/Database/DbWriter.cs`, `tests/CodeIndex.Tests/FileIndexerTests.cs`, `tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`, `README.md`, `DEVELOPER_GUIDE.md`, `CLAUDE.md`. Closes #158.
...
#### 修正
- **拡張子なし shebang スクリプトを推定言語でインデックスするよう修正 (#158)** — `FileIndexer` が、既知の拡張子と完全一致ファイル名だけでインデックス可否を決めないようになった。これらで言語が決まらない場合は先頭行を読み取り、一般的な shebang インタプリタを認識して `libexec/` 配下のような拡張子なしスクリプトも `shell` / `python` / `ruby` / `javascript` / `php` / `lua` / `powershell` としてインデックスに取り込む。フォールバックは truly extensionless なファイルに限定し、full scan / update mode の更新では shebang を失って非対応になった既存 row も purge するようにした。さらに Unix の special file（例: `tool`, `tool.sh`, `Dockerfile` という名前の FIFO）を read/probe 前に除外し、index がブロックしないようにした。言語判定・ディレクトリ走査・full scan / update-mode purge・FIFO ハンドリング・`BuildRecord` の回帰テストを追加し、README / 開発者向けドキュメントも shebang フォールバックと PowerShell の拡張子一覧に同期した。対象: `src/CodeIndex/Indexer/FileIndexer.cs`, `src/CodeIndex/Cli/IndexCommandRunner.cs`, `src/CodeIndex/Database/DbWriter.cs`, `tests/CodeIndex.Tests/FileIndexerTests.cs`, `tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`, `README.md`, `DEVELOPER_GUIDE.md`, `CLAUDE.md`。Closes #158.
