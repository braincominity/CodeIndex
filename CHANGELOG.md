...
#### 修正
- **CLI クエリエラーが次に取るべき行動を案内するよう改善** — query 系コマンドの代表的な stderr 経路で、失敗内容だけでなく次のアクションも出すようにした。query/path の欠落はそのまま再実行できるコマンド形を案内し、不正な数値オプションは warning を出して既定値で続行するのではなく `UsageError` で fail-closed しつつ有効値での再試行例を示し、DB 未作成・オープン失敗時は index の作成／更新を促す。さらに、不正な `--since` など `ParseArgs` 段階の入力エラーは早期に `UsageError` として返し、値を取るオプションは末尾欠落や直後の flag 飲み込みを黙認せず拒否し、`symbols` 以外のコマンドでは想定外の余剰 positional 引数も拒否し、query コマンドと `languages` は canonical usage に載っていないフラグを黙殺せず明示的に拒否し、`mcp` もサーバー起動前に不正・未対応オプションを拒否し、`--since` 未対応コマンドではフィルタを明示的に拒否するようにした。共有 usage 行と README の option table も実装済みオプションに揃え、`validate` は未対応の絞り込みオプションを黙って無視せず明示的に拒否する。対象: `src/CodeIndex/Cli/ConsoleUi.cs`, `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Program.cs`, `tests/CodeIndex.Tests/ConsoleUiTests.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/ProgramCliTests.cs`, `README.md`。
