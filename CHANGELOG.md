...
#### Fixed
- **Streaming CLI JSON keeps empty stdout on zero-hit rows, and docs now list `unused --count`** — `symbols`, `definition`, `references`, `callers`, and `callees` once again keep non-`--count` zero-hit `--json` responses silent so stream consumers can continue treating EOF as "no rows" instead of parsing an ad-hoc envelope object. Help text and README now also document the actual JSON contract more precisely (`search`/`symbols`-style hit streams vs single-object summary/count paths) and add the shipped `unused --count` support to the usage/docs matrix in both English and Japanese. Affected: `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Cli/ConsoleUi.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/ConsoleUiTests.cs`, `README.md`.
...
#### 修正
- **streaming な CLI JSON は 0 件時に再び無出力を維持し、`unused --count` を help/docs に反映** — `symbols`、`definition`、`references`、`callers`、`callees` の非 `--count` `--json` 0 件レスポンスは、再び stdout を空のまま返すようにした。これにより、stream consumer は ad-hoc な envelope object を解釈せずに EOF を「0 行」として扱い続けられる。あわせて help と README は JSON 契約をより正確に説明し（`search` / `symbols` 系の hit stream は JSON lines、summary/count 系は単一オブジェクト）、実装済みだった `unused --count` も英日両方の usage/docs 行列に追加した。対象: `src/CodeIndex/Cli/QueryCommandRunner.cs`、`src/CodeIndex/Cli/ConsoleUi.cs`、`tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`、`tests/CodeIndex.Tests/ConsoleUiTests.cs`、`README.md`。
