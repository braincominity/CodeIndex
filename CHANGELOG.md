...
#### Changed
- **Self-improvement loop now foregrounds truthful delta index refreshes** — Updated `SELF_IMPROVEMENT.md` to prefer `index --commits HEAD` for the normal post-commit path, keep `index --files` limited to known in-place edits or new files, and reserve full `cdidx . --json` scans for checkout-changing git operations or repo-wide stale-file purge. The docs now call out that `--files` alone does not purge old paths from renames/deletes unless those old paths are passed explicitly. Affected: `SELF_IMPROVEMENT.md`.
...
#### 変更
- **自己改善ループで truthful な差分 index refresh を前面化** — `SELF_IMPROVEMENT.md` を更新し、通常の post-commit 経路には rename/delete も追える `index --commits HEAD` を優先し、`index --files` は把握している in-place 編集や新規追加に限定する方針を明記した。あわせて、rename/delete の旧 path は明示しない限り `--files` だけでは purge されないことも追記し、checkout を変える git 操作や repo 全体の stale file 掃除が必要な場合だけ `cdidx . --json` のフルスキャンへ上げるよう整理した。対象: `SELF_IMPROVEMENT.md`。
