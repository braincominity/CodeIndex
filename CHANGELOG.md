...
#### Fixed
- **`impact` now explains class-like and multi-definition zero hits (#107)** — `impact` / MCP `impact_analysis` no longer collapse class-symbol queries into an unhelpful plain zero result. When a class-like query has no symbol-level callers, the read path now falls back to reverse file dependencies so member-only usage still shows blast radius. When the query resolves to multiple definition files (for example partial classes), the zero-result payload explicitly reports that resolution and suggests `deps --path <definition-path> --reverse` or a more specific member query. CLI JSON zero-hit responses now always emit structured payloads instead of staying silent, so AI clients can distinguish real "no impact" from "resolved to class / partial definitions". Affected: `src/CodeIndex/Database/DbReader.cs`, `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Mcp/McpToolHandlers.cs`, `src/CodeIndex/Models/QueryResults.cs`, `tests/CodeIndex.Tests/DbReaderTests.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `README.md`, `DEVELOPER_GUIDE.md`, `CLAUDE.md`. Closes #107.
...
#### 修正
- **`impact` が class 系 / 複数定義の 0 件理由を説明するよう改善 (#107)** — `impact` / MCP `impact_analysis` は、class 系シンボル query を単なる無言の 0 件に潰さなくなった。class 系 query に symbol-level caller が無い場合は、read path が reverse file dependency へフォールバックし、member 経由だけの利用でも blast radius を返す。query が複数の definition file（partial class など）に解決された場合は、0 件 payload にその解決結果を明示し、`deps --path <definition-path> --reverse` またはより具体的な member query を試すヒントも返す。CLI JSON の 0 件応答も今後は必ず structured payload を出力するため、AI クライアントは本当の「影響なし」と「class / partial 定義に解決されたための 0 件」を区別できる。対象: `src/CodeIndex/Database/DbReader.cs`、`src/CodeIndex/Cli/QueryCommandRunner.cs`、`src/CodeIndex/Mcp/McpToolHandlers.cs`、`src/CodeIndex/Models/QueryResults.cs`、`tests/CodeIndex.Tests/DbReaderTests.cs`、`tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`、`tests/CodeIndex.Tests/McpServerTests.cs`、`README.md`、`DEVELOPER_GUIDE.md`、`CLAUDE.md`。Closes #107。
