...
#### Changed
- **README now foregrounds token savings and moves maintainer detail off the landing page** — Slimmed the README sections about repo overview, AI integration, AI feedback, and releases so the front page stays user-focused. Detailed release workflow now lives in `DEVELOPER_GUIDE.md`, the README now explicitly pitches lower AI token consumption, the duplicated freshness prose was removed, and the Japanese MCP tool count was corrected to 23. Affected: `README.md`, `DEVELOPER_GUIDE.md`.
...
#### 変更
- **README でトークン節約価値を前面化し、メンテナ向け詳細をランディングページから移動** — README の「俯瞰」「AI連携」「AIフィードバック」「リリース」各節を薄くして、利用者向けの入口に整理した。詳細なリリース手順は `DEVELOPER_GUIDE.md` へ移し、README では AI のトークン消費削減を明示的に訴求し、重複していた鮮度説明を削除し、日本語版の MCP ツール数を 23 に修正した。対象: `README.md`, `DEVELOPER_GUIDE.md`.
