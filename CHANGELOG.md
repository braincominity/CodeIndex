...
#### Fixed
- **`impact` now explains type-level and multi-definition zero hits without fabricating fallback edges (#107)** — `impact` / MCP `impact_analysis` no longer collapse type-symbol queries into an unhelpful plain zero result, but the new fallback is now restricted to a single resolved `class` / `struct` / `interface` definition and joins only members whose `container_name` matches that resolved type. This prevents false reverse-dependency edges when another type elsewhere defines the same member name, and namespace queries now stay at zero with an explicit hint instead of emitting synthetic file dependencies. When the query resolves to multiple definition files (for example partial classes), the zero-result payload explicitly reports that resolution and suggests `deps --path <definition-path> --reverse` or a more specific member query. CLI JSON zero-hit responses now always emit structured payloads instead of staying silent, so AI clients can distinguish real "no impact" from "resolved to type / partial definitions". Affected: `src/CodeIndex/Database/DbReader.cs`, `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Mcp/McpToolHandlers.cs`, `src/CodeIndex/Models/QueryResults.cs`, `tests/CodeIndex.Tests/DbReaderTests.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `README.md`, `DEVELOPER_GUIDE.md`, `CLAUDE.md`. Closes #107.
...
#### 修正
- **`impact` が型レベル / 複数定義の 0 件理由を説明しつつ偽の fallback edge を作らないよう改善 (#107)** — `impact` / MCP `impact_analysis` は、type symbol query を単なる無言の 0 件に潰さなくなったが、新しい fallback は単一の `class` / `struct` / `interface` 定義にだけ限定し、join 先も `container_name` が解決済み型名に一致する member のみに絞るようにした。これにより、別型に同名 member があるだけで偽の reverse dependency edge を作ることがなくなり、namespace query も synthetic な file dependency を返さず明示ヒント付きの 0 件に留まる。query が複数の definition file（partial class など）に解決された場合は、0 件 payload にその解決結果を明示し、`deps --path <definition-path> --reverse` またはより具体的な member query を試すヒントも返す。CLI JSON の 0 件応答も今後は必ず structured payload を出力するため、AI クライアントは本当の「影響なし」と「型 / partial 定義に解決されたための 0 件」を区別できる。対象: `src/CodeIndex/Database/DbReader.cs`、`src/CodeIndex/Cli/QueryCommandRunner.cs`、`src/CodeIndex/Mcp/McpToolHandlers.cs`、`src/CodeIndex/Models/QueryResults.cs`、`tests/CodeIndex.Tests/DbReaderTests.cs`、`tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`、`tests/CodeIndex.Tests/McpServerTests.cs`、`README.md`、`DEVELOPER_GUIDE.md`、`CLAUDE.md`。Closes #107。
