...
#### Changed
- **Unicode-aware `--exact` name matching across symbols / definition / references / callers / callees / inspect (#86)** — All `--exact` paths (CLI + MCP, including the bundled `inspect` / `analyze_symbol` workflow) now match symbol and reference names through a Unicode-normalized fold (NFKC + invariant lowercase) instead of SQLite's ASCII-only `COLLATE NOCASE`. A new `name_folded` column on `symbols`, plus `symbol_name_folded` / `container_name_folded` on `symbol_references`, are populated by the writer at index time and indexed (`idx_symbols_name_folded`, `idx_symbol_refs_symbol_name_folded`, `idx_symbol_refs_container_name_folded`) so `--exact` queries stay SARGable. Casing pairs that the previous `NOCASE` path missed — `Ä` / `ä`, fullwidth / halfwidth (`Ｒｕｎ` / `Run`), ligatures, compatibility forms — now collapse correctly. Trust is gated by a new `FoldReadyFlag` (bit 2 of `PRAGMA user_version`) which is stamped only after a full-scan index successfully populates every row; legacy DBs without the bit silently fall back to the existing `COLLATE NOCASE` path so ASCII queries keep working until the user runs `cdidx index .`. `FoldReadyFlag` is gated on a runtime backfill check (`DbWriter.AllFoldedColumnsBackfilled`) that verifies every row in `symbols` / `symbol_references` has its folded column populated — an incremental full scan that skips unchanged legacy files will NOT stamp FoldReady (the user is asked to run `--rebuild` to upgrade), and a successful partial update on an already-ready DB DOES restamp FoldReady because every pre-existing row was already folded and every newly-written row also is. Addresses the codex adversarial-review findings on the initial #86 implementation. Affected: `src/CodeIndex/Database/NameFold.cs` (new), `src/CodeIndex/Database/DbContext.cs`, `src/CodeIndex/Database/DbWriter.cs`, `src/CodeIndex/Database/DbReader.cs`, `src/CodeIndex/Database/DbSymbolReader.cs`, `src/CodeIndex/Cli/IndexCommandRunner.cs`, `src/CodeIndex/Mcp/McpToolHandlers.cs`, `tests/CodeIndex.Tests/DbReaderTests.cs`, `tests/CodeIndex.Tests/LegacySchemaMigrationTests.cs`, `README.md`. Closes #86.
...
#### 変更
- **`--exact` の名前マッチを Unicode 対応に拡張（symbols / definition / references / callers / callees / inspect、#86）** — CLI / MCP 双方の全 `--exact` 経路（bundle 系の `inspect` / `analyze_symbol` を含む）が、SQLite の ASCII 限定 `COLLATE NOCASE` ではなく Unicode 折り畳み（NFKC 正規化 + invariant lowercase）でシンボル名・参照名を比較するようになった。新規列 `symbols.name_folded`、`symbol_references.symbol_name_folded` / `container_name_folded` を writer が index 時に埋め、index（`idx_symbols_name_folded`、`idx_symbol_refs_symbol_name_folded`、`idx_symbol_refs_container_name_folded`）を貼ることで `--exact` クエリは SARGable のまま。これまで `NOCASE` で取りこぼしていたケース（`Ä` / `ä`、全角/半角 `Ｒｕｎ` / `Run`、合字、互換形式など）が正しく一致する。信頼性は `PRAGMA user_version` の bit 2 として新たに `FoldReadyFlag` を導入し、full-scan の index が全行を populate して成功した場合のみ stamp する。旧 DB では bit が立たず、reader は既存の `COLLATE NOCASE` 経路に黙ってフォールバックするため ASCII クエリは動作し続ける（Unicode fold が必要なら `cdidx index .` で再 index）。`FoldReadyFlag` の stamp 前には `DbWriter.AllFoldedColumnsBackfilled`（`symbols` / `symbol_references` の folded 列に NULL が残っていないかを SQL で実検証）を必ず通す。unchanged file を skip する incremental full-scan で legacy 行が残っていれば FoldReady は stamp されず（ユーザに `--rebuild` での upgrade を案内）、逆に既に fold-ready だった DB に対する成功 partial update では fold-ready を復元する（既存行も新規行も folded 付きのため invariant は保たれる）。これらは初期 #86 実装に対する codex 指摘への対応。対象: `src/CodeIndex/Database/NameFold.cs`（新規）、`src/CodeIndex/Database/DbContext.cs`、`src/CodeIndex/Database/DbWriter.cs`、`src/CodeIndex/Database/DbReader.cs`、`src/CodeIndex/Database/DbSymbolReader.cs`、`src/CodeIndex/Cli/IndexCommandRunner.cs`、`src/CodeIndex/Mcp/McpToolHandlers.cs`、`tests/CodeIndex.Tests/DbReaderTests.cs`、`tests/CodeIndex.Tests/LegacySchemaMigrationTests.cs`、`README.md`。Closes #86.
