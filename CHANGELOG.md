...
#### Fixed
- **Extensionless shebang scripts are now indexed under their inferred language (#158)** — `FileIndexer` no longer limits indexability to known extensions and exact filename matches. When those checks miss, it now reads the first line and recognizes common shebang interpreters so extensionless scripts under directories like `libexec/` still enter the index as `shell`, `python`, `ruby`, `javascript`, `php`, `lua`, or `powershell`. Added regression coverage for language detection, directory scanning, and `BuildRecord`, and updated user/developer docs to document the shebang fallback plus the full PowerShell extension set. Affected: `src/CodeIndex/Indexer/FileIndexer.cs`, `tests/CodeIndex.Tests/FileIndexerTests.cs`, `README.md`, `DEVELOPER_GUIDE.md`, `CLAUDE.md`. Closes #158.
...
#### 修正
- **拡張子なし shebang スクリプトを推定言語でインデックスするよう修正 (#158)** — `FileIndexer` が、既知の拡張子と完全一致ファイル名だけでインデックス可否を決めないようになった。これらで言語が決まらない場合は先頭行を読み取り、一般的な shebang インタプリタを認識して `libexec/` 配下のような拡張子なしスクリプトも `shell` / `python` / `ruby` / `javascript` / `php` / `lua` / `powershell` としてインデックスに取り込む。言語判定・ディレクトリ走査・`BuildRecord` の回帰テストを追加し、README / 開発者向けドキュメントも shebang フォールバックと PowerShell の拡張子一覧に同期した。対象: `src/CodeIndex/Indexer/FileIndexer.cs`, `tests/CodeIndex.Tests/FileIndexerTests.cs`, `README.md`, `DEVELOPER_GUIDE.md`, `CLAUDE.md`。Closes #158.
