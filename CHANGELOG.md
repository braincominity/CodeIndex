...
- **`unused` now separates high-confidence private hits from exported/config suspects (#105)** — `GetUnusedSymbols` now classifies results into `likely_unused_private`, `maybe_unused_nonpublic`, `public_or_exported_no_refs`, and `reflection_or_config_suspect`, orders the buckets so high-confidence private hits surface first under small limits, and annotates every unused result with `unused_bucket`, `unused_confidence`, and `unused_reason`. CLI human output is grouped by bucket, CLI JSON now includes `bucket_counts`, and MCP `unused_symbols` mirrors the same structure for downstream ranking. Affected: `src/CodeIndex/Database/DbSymbolReader.cs`, `src/CodeIndex/Models/QueryResults.cs`, `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Mcp/McpToolHandlers.cs`, `src/CodeIndex/Mcp/McpToolDefinitions.cs`, `tests/CodeIndex.Tests/DbReaderTests.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `README.md`. Closes #105.
...
- **`unused` が高信頼 private 候補と exported/config suspect を分離して返すよう改善 (#105)** — `GetUnusedSymbols` が結果を `likely_unused_private`、`maybe_unused_nonpublic`、`public_or_exported_no_refs`、`reflection_or_config_suspect` に分類し、高信頼 private 候補が小さい `--limit` でも先頭に来るよう bucket 順で並べるようになった。各結果には `unused_bucket`、`unused_confidence`、`unused_reason` が付き、CLI の人間向け出力は bucket ごとにグループ化、CLI JSON は `bucket_counts` を追加、MCP `unused_symbols` も同じ構造を返す。対象: `src/CodeIndex/Database/DbSymbolReader.cs`、`src/CodeIndex/Models/QueryResults.cs`、`src/CodeIndex/Cli/QueryCommandRunner.cs`、`src/CodeIndex/Mcp/McpToolHandlers.cs`、`src/CodeIndex/Mcp/McpToolDefinitions.cs`、`tests/CodeIndex.Tests/DbReaderTests.cs`、`tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`、`tests/CodeIndex.Tests/McpServerTests.cs`、`README.md`。Closes #105。
