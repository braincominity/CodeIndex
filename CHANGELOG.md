...
#### Changed
- **Docs now distinguish production dependencies from test-only packages** — Clarified in `CLAUDE.md`, `DEVELOPER_GUIDE.md`, and `TESTING_GUIDE.md` that the "one dependency" rule applies to the shipping product under `src/CodeIndex`, while the test harness may use separate test-only packages under `tests/CodeIndex.Tests/`. Affected: `CLAUDE.md`, `DEVELOPER_GUIDE.md`, `TESTING_GUIDE.md`.
- **README now foregrounds token savings and moves maintainer detail off the landing page** — Slimmed the README sections about repo overview, AI integration, AI feedback, and releases so the front page stays user-focused. Detailed release workflow now lives in `DEVELOPER_GUIDE.md`, `MAINTAINERS.md` now points straight to that canonical section instead of looping back through README, the README now explicitly pitches lower AI token consumption, the duplicated freshness prose was removed, and the Japanese MCP tool count was corrected to 23. Affected: `README.md`, `DEVELOPER_GUIDE.md`, `MAINTAINERS.md`.
...
- **`unused` now separates private hits from exported/config suspects without overstating confidence (#105)** — `GetUnusedSymbols` now classifies results into `likely_unused_private`, `maybe_unused_nonpublic`, `public_or_exported_no_refs`, and `reflection_or_config_suspect`, but narrows the suspect bucket to public/exported properties with explicit config/settings/options context or, in C#, nearby serializer/ORM-style attributes instead of broad keyword matches on arbitrary methods or classes. Private hits are no longer labeled `high` confidence solely because indexed references are absent; they degrade to `medium` until stronger corroboration exists. Returned pages now use bounded overfetch plus cross-bucket diversification before `--limit` is applied so noisy private hits are less likely to crowd out every lower bucket without silently capping large `--limit` requests, unsupported `--lang` filters return empty pages with trust hints instead of false unused hits, and missing `chunks` tables now degrade attribute-based suspect detection instead of aborting the whole query. CLI human output is grouped by bucket, CLI JSON exposes page-scoped `returned_bucket_counts`, per-symbol `unused_bucket` / `unused_confidence` / `unused_reason`, and `graph_supported` / `graph_support_reason` when a language filter is supplied, even on zero-result pages; `unused --json` returns those empty-page payloads with exit code `0` so machine clients can consume them as normal structured results. MCP `unused_symbols` mirrors the same trust metadata and now marks missing `symbol_references` tables as degraded instead of looking like an authoritative zero. Affected: `src/CodeIndex/Database/DbSymbolReader.cs`, `src/CodeIndex/Models/QueryResults.cs`, `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Mcp/McpToolHandlers.cs`, `src/CodeIndex/Mcp/McpToolDefinitions.cs`, `tests/CodeIndex.Tests/DbReaderTests.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `README.md`. Closes #105.

#### Fixed
- **Streaming CLI JSON keeps authoritative zero-hit stdout empty while degraded graph zeroes stay structured, and docs now list `unused --count`** — `symbols`, `definition`, `references`, `callers`, and `callees` once again keep non-`--count` authoritative zero-hit `--json` responses silent so stream consumers can continue treating EOF as "no rows" instead of parsing an ad-hoc envelope object. Missing-graph-table zero results remain explicit JSON payloads (`graph_table_available=false`, `degraded=true`) so machine clients can still distinguish legacy/read-only degradation from a trustworthy empty result. Help text and README now also document the actual JSON contract more precisely (`search`/`symbols`-style hit streams vs single-object summary/count paths / degraded zero envelopes) and add the shipped `unused --count` support to the usage/docs matrix in both English and Japanese. Affected: `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Cli/ConsoleUi.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/ConsoleUiTests.cs`, `README.md`.
- **`backfill-fold` now rejects blank or nonexistent DB targets before schema init without blocking legacy upgrades** — `backfill-fold` / `backfill_fold` now validate that `--db` already points at an existing CodeIndex database before any schema initialization runs. Blank files and nonexistent `file:` URIs are rejected instead of creating a fresh empty schema and falsely reporting `fold_ready=true`, while legitimate legacy DBs that have the core CodeIndex tables but are still missing newer metadata tables such as `codeindex_meta` remain valid upgrade targets. Help, MCP tool descriptions, and README examples now call out that backfill is an in-place upgrade for an existing index only. Affected: `src/CodeIndex/Database/DbContext.cs`, `src/CodeIndex/Cli/IndexCommandRunner.cs`, `src/CodeIndex/Mcp/McpToolHandlers.cs`, `src/CodeIndex/Mcp/McpToolDefinitions.cs`, `src/CodeIndex/Cli/ConsoleUi.cs`, `tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `README.md`.
- **Impact JSON zero-result tests no longer lose fields due to cross-test SQLite pool resets (#138)** — `TestProjectHelper.DeleteDirectory()` no longer clears SQLite pools on every temp-project cleanup, and the test classes that intentionally call `SqliteConnection.ClearAllPools()` now share a non-parallel xUnit collection so process-global pool resets cannot race unrelated SQLite tests. `QueryCommandRunnerTests` also re-run the affected partial-class and import-only `impact --json` zero-result scenarios across repeated temp-project lifecycles and assert the common zero-result envelope before the impact-specific fields, so dropped JSON keys are caught directly. `TESTING_GUIDE.md` now documents the pool-sensitive collection rule. Affected: `tests/CodeIndex.Tests/TestProjectHelper.cs`, `tests/CodeIndex.Tests/SqlitePoolSensitiveCollection.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/DatabaseTests.cs`, `tests/CodeIndex.Tests/DbReaderTests.cs`, `tests/CodeIndex.Tests/LegacySchemaMigrationTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`, `tests/CodeIndex.Tests/DbRecoveryTests.cs`, `tests/CodeIndex.Tests/PerformanceTests.cs`, `tests/CodeIndex.Tests/ConcurrencyTests.cs`, `TESTING_GUIDE.md`. Closes #138.
- **Command-specific exact aliases reduce the `--exact` semantic trap (#84)** — Added explicit non-breaking aliases so users and AI clients can say what kind of exactness they want without overloading one flag name. CLI now supports `--exact-substring` for `search` and `--exact-name` for `symbols` / `definition` / `references` / `callers` / `callees` / `inspect`, while legacy `--exact` remains as backward-compatible shorthand. MCP tool schemas now expose `exactSubstring` on `search` and `exactName` on symbol/graph/analyze tools, with handlers rejecting the wrong alias instead of silently accepting an ambiguous request. Help text and README now recommend the explicit aliases. Affected: `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Cli/ConsoleUi.cs`, `src/CodeIndex/Mcp/McpToolDefinitions.cs`, `src/CodeIndex/Mcp/McpToolHandlers.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `tests/CodeIndex.Tests/ConsoleUiTests.cs`, `README.md`. Closes #84.
...
#### 変更
- **ドキュメントで本番依存と test-only package を明確に区別** — `CLAUDE.md`、`DEVELOPER_GUIDE.md`、`TESTING_GUIDE.md` において、「依存は1個だけ」というルールが `src/CodeIndex` の出荷物に適用される production dependency 方針であり、`tests/CodeIndex.Tests/` の test-only package とは別であることを明記した。対象: `CLAUDE.md`、`DEVELOPER_GUIDE.md`、`TESTING_GUIDE.md`。
...
- **`unused` が confidence を過大表示せずに private 候補と exported/config suspect を分離して返すよう改善 (#105)** — `GetUnusedSymbols` が結果を `likely_unused_private`、`maybe_unused_nonpublic`、`public_or_exported_no_refs`、`reflection_or_config_suspect` に分類するようになり、suspect bucket は任意のメソッド名やクラス名に対する広いキーワード一致ではなく、public/exported な property かつ config/settings/options の明示的文脈、または C# で近傍に serializer/ORM 系 attribute がある場合に限定した。indexed reference が無いだけの private 候補は `high` とはラベルせず `medium` に留める。返却ページは `--limit` の前に bucket 間で分散されるため、ノイズの多い private bucket だけで他 bucket が埋もれにくい。unsupported `--lang` は trust hint 付きの空結果を返し、`chunks` テーブル欠落時も attribute 判定を安全に無効化して query 全体は落とさない。CLI の人間向け出力は bucket ごとにグループ化し、CLI JSON は返却ページ単位の `returned_bucket_counts`、シンボルごとの `unused_bucket` / `unused_confidence` / `unused_reason`、さらに `--lang` 指定時の `graph_supported` / `graph_support_reason` を返す。MCP `unused_symbols` は同じ情報を per-symbol では camelCase、集計は `returned_bucket_counts` のままで返す。対象: `src/CodeIndex/Database/DbSymbolReader.cs`、`src/CodeIndex/Models/QueryResults.cs`、`src/CodeIndex/Cli/QueryCommandRunner.cs`、`src/CodeIndex/Mcp/McpToolHandlers.cs`、`src/CodeIndex/Mcp/McpToolDefinitions.cs`、`tests/CodeIndex.Tests/DbReaderTests.cs`、`tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`、`tests/CodeIndex.Tests/McpServerTests.cs`、`README.md`。Closes #105。

#### 修正
- **streaming な CLI JSON は authoritative な 0 件時のみ無出力を維持し、degraded な graph 0 件は JSON を返すよう整理。`unused --count` も help/docs に反映** — `symbols`、`definition`、`references`、`callers`、`callees` の非 `--count` `--json` 0 件レスポンスは、trust できる 0 件なら再び stdout を空のまま返す。一方、`symbol_references` 欠損による degraded な 0 件は `graph_table_available=false` / `degraded=true` 付きの JSON payload を返すため、machine client は legacy/read-only の縮退と本物の空結果を区別できる。あわせて help と README は JSON 契約をより正確に説明し（`search` / `symbols` 系の hit stream は JSON lines、summary/count 系や degraded zero は単一オブジェクト）、実装済みだった `unused --count` も英日両方の usage/docs 行列に追加した。対象: `src/CodeIndex/Cli/QueryCommandRunner.cs`、`src/CodeIndex/Cli/ConsoleUi.cs`、`tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`、`tests/CodeIndex.Tests/ConsoleUiTests.cs`、`README.md`。
- **`backfill-fold` が schema 初期化前に空DB / 不存在DBを拒否しつつ legacy upgrade は塞がないよう修正** — `backfill-fold` / `backfill_fold` は `--db` が既存の CodeIndex DB を指していることを schema 初期化前に検証するようになった。空ファイルや不存在の `file:` URI を指定しても、新しい空スキーマを作って `fold_ready=true` と誤報することはない。一方で、`files` / `chunks` / `symbols` などの core table を持つ正当な legacy DB は、`codeindex_meta` のような新しめの metadata table が未作成でも引き続き upgrade 対象として受け入れる。help、MCP ツール説明、README の例も、backfill が既存 index 専用の in-place upgrade であることを明示した。対象: `src/CodeIndex/Database/DbContext.cs`、`src/CodeIndex/Cli/IndexCommandRunner.cs`、`src/CodeIndex/Mcp/McpToolHandlers.cs`、`src/CodeIndex/Mcp/McpToolDefinitions.cs`、`src/CodeIndex/Cli/ConsoleUi.cs`、`tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`、`tests/CodeIndex.Tests/McpServerTests.cs`、`README.md`。
- **cross-test な SQLite pool reset による `impact` JSON zero-result テストの欠落を防止 (#138)** — `TestProjectHelper.DeleteDirectory()` が temp project cleanup のたびに SQLite pool を全解放していた問題をやめ、さらに `SqliteConnection.ClearAllPools()` を意図的に使うテストクラス群を non-parallel な xUnit collection にまとめた。これにより process-global な pool reset が無関係な SQLite テストへ並列に干渉しなくなった。`QueryCommandRunnerTests` には partial class / import-only の `impact --json` zero-result シナリオを temp project の繰り返し生成・削除つきで再実行し、共通 zero-result envelope を先に検証する回帰も追加。`TESTING_GUIDE.md` にも pool-sensitive collection の運用を追記。対象: `tests/CodeIndex.Tests/TestProjectHelper.cs`, `tests/CodeIndex.Tests/SqlitePoolSensitiveCollection.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/DatabaseTests.cs`, `tests/CodeIndex.Tests/DbReaderTests.cs`, `tests/CodeIndex.Tests/LegacySchemaMigrationTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`, `tests/CodeIndex.Tests/DbRecoveryTests.cs`, `tests/CodeIndex.Tests/PerformanceTests.cs`, `tests/CodeIndex.Tests/ConcurrencyTests.cs`, `TESTING_GUIDE.md`。Closes #138。
- **コマンド別の exact alias で `--exact` の意味衝突を緩和 (#84)** — exactness の種類を 1 つのフラグ名に押し込めずに済むよう、非破壊の explicit alias を追加した。CLI は `search` 向けに `--exact-substring`、`symbols` / `definition` / `references` / `callers` / `callees` / `inspect` 向けに `--exact-name` を受け付け、従来の `--exact` は後方互換の短縮形として残す。MCP の tool schema も `search` に `exactSubstring`、symbol / graph / analyze 系に `exactName` を追加し、handler は不適切な alias を曖昧に受け入れず明示エラーにする。help text と README も explicit alias 推奨に更新。対象: `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Cli/ConsoleUi.cs`, `src/CodeIndex/Mcp/McpToolDefinitions.cs`, `src/CodeIndex/Mcp/McpToolHandlers.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `tests/CodeIndex.Tests/ConsoleUiTests.cs`, `README.md`。Closes #84。
