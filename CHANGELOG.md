...
- **Zig, CSS/SCSS, Shell reference extraction** — Three more languages now support call-graph queries. Zig uses `//` comments. CSS has no line comments (block comments `/* */` already handled). Shell uses `#` comments. Affected: `ReferenceExtractor.cs`.
...
- **Zig、CSS/SCSS、Shell 参照抽出** — 3言語追加でコールグラフクエリが利用可能に。Zig は `//` コメント。CSS には行コメントなし（ブロックコメント `/* */` は対応済み）。Shell は `#` コメント。対象: `ReferenceExtractor.cs`。
