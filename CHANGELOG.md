...
#### Changed
- **Self-improvement loop now foregrounds delta index refreshes** — Updated `SELF_IMPROVEMENT.md` to prefer `index --files` for known local edits and `index --commits HEAD` for the normal post-commit path, while reserving full `cdidx . --json` scans for checkout-changing git operations or repo-wide stale-file purge. This keeps the rebuild-and-refresh contract intact without presenting full rescans as the default for every commit on large repositories. Affected: `SELF_IMPROVEMENT.md`.
...
#### 変更
- **自己改善ループで差分 index refresh を前面化** — `SELF_IMPROVEMENT.md` を更新し、把握している手元編集には `index --files`、通常の post-commit 経路には `index --commits HEAD` を優先し、checkout を変える git 操作や repo 全体の stale file 掃除が必要な場合だけ `cdidx . --json` のフルスキャンへ上げる方針を明記した。これにより、大規模リポジトリでも「毎コミットで再ビルドして refresh する」契約を維持しつつ、常に full rescan が既定のように読めないようにした。対象: `SELF_IMPROVEMENT.md`。
