...
#### Fixed
- **Zero-result JSON queries now always emit a payload with explicit freshness availability (#108)** — Zero-hit `--json` paths across the CLI query runner and MCP tool handlers no longer fall through with empty stdout, and their freshness contract now distinguishes empty indexes from legacy/read-only degradation. `search`, `files`, `symbols`, `definition`, `references`, `callers`, `callees`, `deps`, `unused`, `hotspots`, and `impact` now all emit a machine-readable zero-result envelope that includes the command-appropriate empty array field plus `indexed_file_count`, `indexed_at`, and `freshness_available`; `indexed_at:null` with `freshness_available=true` means an empty index, while `freshness_available=false` adds `freshness_degraded_reason` for DBs that cannot expose freshness timestamps. Existing graph/exact-match metadata stays intact, and `impact` preserves its query/max-depth envelope even when the caller chain is empty. Added CLI/MCP regression coverage for populated indexes, empty indexes, and legacy read-only databases without `files.indexed_at`. Affected: `src/CodeIndex/Cli/QueryCommandRunner.cs`, `src/CodeIndex/Database/DbReader.cs`, `src/CodeIndex/Mcp/McpToolHandlers.cs`, `src/CodeIndex/Models/QueryResults.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `README.md`, `DEVELOPER_GUIDE.md`, `CLAUDE.md`. Closes #108.
...
#### 修正
- **ゼロ件 JSON クエリでも常に payload を返し、鮮度取得可否も明示するようにした (#108)** — CLI の query runner と MCP tool handler の `--json` / JSON ゼロ件経路が空 stdout のまま終了することはなくなり、鮮度契約も空インデックスと legacy/read-only 劣化を区別できるようになった。`search`、`files`、`symbols`、`definition`、`references`、`callers`、`callees`、`deps`、`unused`、`hotspots`、`impact` は、コマンドごとの空配列フィールドに加えて `indexed_file_count`、`indexed_at`、`freshness_available` を含む machine-readable なゼロ件 envelope を返す。`freshness_available=true` で `indexed_at:null` なら空インデックス、`freshness_available=false` なら鮮度 timestamp を出せない DB であり、理由は `freshness_degraded_reason` に入る。既存の graph / exact-match メタデータは維持され、`impact` も caller chain が空のときに query / max-depth envelope を保つ。`files.indexed_at` を欠く legacy read-only DB も含めた CLI/MCP の回帰テストを追加。対象: `src/CodeIndex/Cli/QueryCommandRunner.cs`、`src/CodeIndex/Database/DbReader.cs`、`src/CodeIndex/Mcp/McpToolHandlers.cs`、`src/CodeIndex/Models/QueryResults.cs`、`tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`、`tests/CodeIndex.Tests/McpServerTests.cs`、`README.md`、`DEVELOPER_GUIDE.md`、`CLAUDE.md`。Closes #108。
