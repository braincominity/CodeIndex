...
#### Fixed
- **Zero-result CLI JSON queries now always emit a payload (#108)** — Zero-hit `--json` paths across the query runner no longer fall through with empty stdout. `search`, `files`, `symbols`, `definition`, `references`, `callers`, `callees`, `deps`, `unused`, `hotspots`, and `impact` now all emit a machine-readable zero-result envelope that includes the command-appropriate empty array field plus `indexed_file_count` / `indexed_at` freshness metadata so AI clients can distinguish "authoritative zero hits" from "no JSON was produced". Existing graph/exact-match metadata stays intact, and `impact` preserves its query/max-depth envelope even when the caller chain is empty. Added CLI regression coverage across text, symbol, graph, and aggregate query families. Affected: `src/CodeIndex/Cli/QueryCommandRunner.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`. Closes #108.
...
#### 修正
- **ゼロ件の CLI JSON クエリでも常に payload を返すようにした (#108)** — Query runner の `--json` ゼロ件経路が空 stdout のまま終了することはなくなった。`search`、`files`、`symbols`、`definition`、`references`、`callers`、`callees`、`deps`、`unused`、`hotspots`、`impact` は、コマンドごとの空配列フィールドに加えて `indexed_file_count` / `indexed_at` の鮮度メタデータを含む machine-readable なゼロ件 envelope を必ず返すようになり、AI クライアントは「権威ある 0 件」と「JSON 自体が出なかった」を区別できる。既存の graph / exact-match メタデータは維持され、`impact` も caller chain が空のときに query / max-depth envelope を保つ。テキスト系・シンボル系・グラフ系・集計系の CLI 回帰テストを追加。対象: `src/CodeIndex/Cli/QueryCommandRunner.cs`、`tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`。Closes #108。
