...
#### Changed
- **Truthful delta-refresh guidance now covers self-improvement docs, public docs, help text, and regressions** — Updated `SELF_IMPROVEMENT.md`, `README.md`, and CLI help text to prefer `index --commits HEAD` for the normal post-commit path, keep `index --files` limited to known in-place edits or new files, and call out that `--files` alone does not purge old rename/delete paths unless those old paths are passed explicitly. Added regression coverage for git rename commit path discovery plus the corresponding `--commits` vs `--files` index-update behavior so the documented guidance stays enforced. Affected: `SELF_IMPROVEMENT.md`, `README.md`, `src/CodeIndex/Cli/ConsoleUi.cs`, `tests/CodeIndex.Tests/ConsoleUiTests.cs`, `tests/CodeIndex.Tests/GitHelperTests.cs`, `tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`.
...
#### 変更
- **truthful な差分 refresh ガイダンスを自己改善文書・公開文書・help・回帰テストまで展開** — `SELF_IMPROVEMENT.md`、`README.md`、CLI help text を更新し、通常の post-commit 経路では rename/delete も追える `index --commits HEAD` を優先し、`index --files` は把握している in-place 編集や新規追加に限定する方針を明記した。あわせて、rename/delete の旧 path は明示しない限り `--files` だけでは purge されないことも追記し、その契約を保つために git rename commit の path 抽出と `--commits` / `--files` の update-mode 挙動を固定する回帰テストも追加した。対象: `SELF_IMPROVEMENT.md`, `README.md`, `src/CodeIndex/Cli/ConsoleUi.cs`, `tests/CodeIndex.Tests/ConsoleUiTests.cs`, `tests/CodeIndex.Tests/GitHelperTests.cs`, `tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`。
