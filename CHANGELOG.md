...
#### Fixed
- **Extensionless shebang scripts are now indexed under their inferred language (#158)** — `FileIndexer` no longer limits indexability to known extensions and exact filename matches. When those checks miss, it now reads the first line and recognizes common shebang interpreters so extensionless scripts under directories like `libexec/` still enter the index as `shell`, `python`, `ruby`, `javascript`, `php`, `lua`, or `powershell`. The fallback stays limited to truly extensionless files, only accepts real first-column shebangs (BOM allowed, leading spaces/tabs rejected), full-scan/update-mode refreshes now purge previously indexed rows when those files lose their shebang and become non-indexable, unreadable paths now degrade the run to partial instead of deleting existing indexed rows or advertising the index as fully ready, and scan/probe failures now report actionable file or directory paths in partial-run errors. Unix special files (for example FIFOs named `tool`, `tool.sh`, or `Dockerfile`) are skipped before any read/probe path can block indexing. Added regression coverage for language detection, directory scanning, full-scan/update-mode purge behavior, unreadable-path handling, FIFO handling, and `BuildRecord`, and updated user/developer docs to document the shebang fallback plus the full PowerShell extension set. Affected: `src/CodeIndex/Indexer/FileIndexer.cs`, `src/CodeIndex/Cli/IndexCommandRunner.cs`, `src/CodeIndex/Database/DbWriter.cs`, `tests/CodeIndex.Tests/FileIndexerTests.cs`, `tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`, `README.md`, `DEVELOPER_GUIDE.md`, `CLAUDE.md`. Closes #158.
