...
#### Fixed
- **Action-oriented stderr guidance outside QueryCommandRunner (#139)** — Index CLI failures now say what to do next for missing directories, invalid `--rebuild` update combos, backfill-fold failures, commit-resolution failures, and partial indexing runs. MCP server stderr now tells clients whether to split requests, fix JSON framing, refresh the index, or retry after a skipped request, and GitHub suggestion submission failures now explain that the suggestion stays local and which credentials/network checks to make before retrying. Added regression coverage for CLI guidance plus formatter-level tests for MCP and GitHub reporter wording. Affected: `src/CodeIndex/Cli/IndexCommandRunner.cs`, `src/CodeIndex/Mcp/McpServer.cs`, `src/CodeIndex/Cli/GitHubIssueReporter.cs`, `tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `tests/CodeIndex.Tests/GitHubIssueReporterTests.cs`. Closes #139.
...
#### 修正
- **QueryCommandRunner 外の stderr を次アクション付きに改善 (#139)** — index CLI の失敗時メッセージが、存在しないディレクトリ、`--rebuild` と update mode の不正な組み合わせ、backfill-fold の失敗、commit 解決失敗、部分的な index 失敗に対して、次に何をすべきかを明示するようになった。MCP server の stderr も、巨大リクエストなら分割、JSON framing エラーなら 1 行 1 JSON-RPC で再送、tool 実行失敗なら引数修正や再インデックス後の再試行といった回復手順を案内する。GitHub suggestion 送信失敗時も、提案がローカルに残ることと、`CDIDX_GITHUB_TOKEN`・権限・ネットワークを確認してから再試行すべきことを説明するようにした。CLI 向けの回帰テストと、MCP / GitHub reporter の wording を固定する formatter-level テストも追加。対象: `src/CodeIndex/Cli/IndexCommandRunner.cs`、`src/CodeIndex/Mcp/McpServer.cs`、`src/CodeIndex/Cli/GitHubIssueReporter.cs`、`tests/CodeIndex.Tests/IndexCommandRunnerTests.cs`、`tests/CodeIndex.Tests/McpServerTests.cs`、`tests/CodeIndex.Tests/GitHubIssueReporterTests.cs`。Closes #139。
