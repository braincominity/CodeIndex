...
#### Fixed
- **CLI query errors now tell the user what to do next** — Common query-side stderr paths now add actionable hints instead of only stating the failure. Missing query/path arguments point to a concrete next command shape, invalid numeric options explain how to retry with a valid value, and missing/failed database opens now tell users and AI agents to create or refresh the index before rerunning. Invalid `ParseArgs` inputs such as malformed `--since` values now fail fast with `UsageError`, query commands that do not implement `--since` now reject it explicitly instead of silently ignoring the filter, and shared usage lines now stay aligned with the actual implemented options. `validate` also rejects unsupported narrowing flags instead of silently ignoring them. Affected: `src/CodeIndex/Cli/ConsoleUi.cs`, `src/CodeIndex/Cli/QueryCommandRunner.cs`, `tests/CodeIndex.Tests/ConsoleUiTests.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`.
...
#### 修正
- **CLI クエリエラーが次に取るべき行動を案内するよう改善** — query 系コマンドの代表的な stderr 経路で、失敗内容だけでなく次のアクションも出すようにした。query/path の欠落はそのまま再実行できるコマンド形を案内し、数値オプションの不正値は有効値での再試行例を示し、DB 未作成・オープン失敗時は index の作成／更新を促す。さらに、不正な `--since` など `ParseArgs` 段階の入力エラーは早期に `UsageError` として返し、`--since` 未対応の query コマンドではフィルタを黙殺せず明示的に拒否するようにした。共有 usage 行も実装済みオプションと揃え、`validate` は未対応の絞り込みオプションを黙って無視せず明示的に拒否するようにした。対象: `src/CodeIndex/Cli/ConsoleUi.cs`, `src/CodeIndex/Cli/QueryCommandRunner.cs`, `tests/CodeIndex.Tests/ConsoleUiTests.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`。
