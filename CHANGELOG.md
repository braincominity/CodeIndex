...
#### Fixed
- **Impact JSON zero-result tests no longer lose fields due to temp-project cleanup pool resets (#138)** — `TestProjectHelper.DeleteDirectory()` no longer clears SQLite pools on every temp-project cleanup, because that process-global reset could interfere with unrelated tests running in parallel and make `impact --json` zero-result payload assertions intermittently parse incomplete output. Pool clearing is now limited to Windows-only delete retries after an actual cleanup failure, and `QueryCommandRunnerTests` now re-run the affected partial-class and import-only impact JSON scenarios across repeated temp-project lifecycles to lock the payload shape down. `TESTING_GUIDE.md` now documents the narrower helper behavior. Affected: `tests/CodeIndex.Tests/TestProjectHelper.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `TESTING_GUIDE.md`. Closes #138.
...
#### 修正
- **temp project cleanup の pool reset が `impact` JSON zero-result テストを不安定化させなくなった (#138)** — `TestProjectHelper.DeleteDirectory()` が temp project cleanup のたびに SQLite pool を全解放していたため、並列実行中の無関係なテストへ process-global な干渉が起き、`impact --json` の zero-result payload assertion が不完全な出力を読んで落ちることがあった。pool 解放は、実際に cleanup 失敗が起きたときの Windows 専用リトライ経路に限定し、`QueryCommandRunnerTests` には partial class / import-only の `impact` JSON シナリオを temp project の繰り返し生成・削除つきで再実行する回帰を追加した。`TESTING_GUIDE.md` も helper の新しい挙動に合わせて更新。対象: `tests/CodeIndex.Tests/TestProjectHelper.cs`, `tests/CodeIndex.Tests/QueryCommandRunnerTests.cs`, `TESTING_GUIDE.md`。Closes #138。
