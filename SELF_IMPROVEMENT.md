...
5. commit it
6. rebuild `cdidx`
7. refresh `.cdidx/codeindex.db` with the newly built binary, usually via delta updates first
...
### 3. Check or refresh the index with the freshly built binary
...
> **Maintainer と認可オペレーター向け** — [MAINTAINERS.md#maintainer-と認可オペレーター向け](MAINTAINERS.md#maintainer-と認可オペレーター向け) を参照してください。エンドユーザーはこのドキュメントを読む必要はありません。
...
5. コミットする
6. `cdidx` を再ビルドする
7. その新しいバイナリで `.cdidx/codeindex.db` を更新する（通常は差分更新を優先する）
...
### 3. そのビルド済みバイナリでインデックスを確認または更新する
