...
public static class IndexCommandRunner
{
    ...
    public static int RunBackfillFold(string[] cmdArgs, JsonSerializerOptions jsonOptions)
    {
        ...
        try
        {
            ...
            // Missing or mismatched fold_key_version means persisted keys may have been
            // generated by a different fold algorithm, so refresh every row from source names.
            // fold_key_version 未記録 / 不一致時は全行再計算して version skew を解消する。
            ...
            else
            {
                ...
                if (rewriteAll)
                    Console.WriteLine("  mode:               full folded-key refresh (fold_key_version missing or mismatched)");
        ...
        JsonSerializerOptions jsonOptions)
    {
        ...
        if (errors == 0)
        {
            ...
            {
                ConsoleUi.PrintWarning("--exact Unicode fold path not stamped: legacy rows without name_folded remain. Run `cdidx backfill-fold` to upgrade without reparsing files, or `cdidx index . --rebuild` for a full rebuild.");
            }
        }
        stopwatch.Stop();
        var (totalFiles, totalChunks, totalSymbols, totalReferences) = writer.GetCounts();
