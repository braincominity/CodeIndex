...
public partial class McpServer
{
    ...
    private JsonNode HandleToolsList(JsonNode? id)
    {
        var tools = new JsonArray
        {
            ...
            CreateToolDefinition(
                "definition",
                "Resolve symbol definitions with definition ranges, signatures, and optional body content. For exact matches, use `exactName`; `exact` is the legacy alias documented in USER_GUIDE.md's flag compatibility table. / 定義範囲、シグネチャ、必要に応じて本体内容付きでシンボル定義を解決。完全一致には `exactName` を使う。`exact` は USER_GUIDE.md の flag compatibility table に記載された legacy alias。",
                new JsonObject
                {
                    ["type"] = "object",
                    ["properties"] = new JsonObject
            ...
            CreateToolDefinition(
                "symbols",
                "Search for code symbols (functions, classes, interfaces, imports) by name pattern. For exact matches, use `exactName`; `exact` is the legacy alias documented in USER_GUIDE.md's flag compatibility table. / シンボル（関数、クラス、インターフェース、import）を名前パターンで検索。完全一致には `exactName` を使う。`exact` は USER_GUIDE.md の flag compatibility table に記載された legacy alias。",
                new JsonObject
                {
                    ["type"] = "object",
                    ["properties"] = new JsonObject
