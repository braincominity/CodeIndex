    ...
    private JsonNode HandleToolsList(JsonNode? id)
    {
        var tools = new JsonArray
        {
            CreateToolDefinition(
                "search",
                "Full-text search across indexed code chunks using FTS5. Returns compact match-centered snippets with line metadata. CJK tokens (Han, Hiragana, Katakana, Hangul, and halfwidth Katakana) in the literal-safe path are auto-upgraded to FTS5 prefix phrases so `search 計算` matches content containing `計算する` without needing `rawQuery`. The CJK prefix promotion is unconditional, so `search 計算する` also returns chunks containing `計算する追加` — use the `exact` flag for strict equality on CJK substrings. Non-CJK tokens — including Latin-diacritic (`naïve`), Greek, Cyrillic, and emoji-mixed text — skip the CJK prefix path to avoid over-widening. Emoji-mixed tokens cannot be distinguished from their plain ASCII counterpart at the FTS layer (unicode61 drops the emoji on both index and query side — `foo🎉` is FTS-equivalent to `foo`), and pure emoji substring search is 0-result for the same reason; use `exact` when emoji identity matters. / FTS5を使ったコードチャンクの全文検索。一致中心の軽量スニペットと行メタデータを返す。literal-safe 経路では CJK トークン（漢字・ひらがな・カタカナ・ハングル・半角カナ）のみ自動で FTS5 prefix phrase に昇格するため、`search 計算` は `rawQuery` なしでも `計算する` を含むコードにマッチする。prefix 昇格は CJK トークンに対して無条件なので `search 計算する` は `計算する追加` も返す — 厳密一致が必要なら `exact` フラグを使う。CJK 以外の非 ASCII トークン（`naïve` 等の Latin-diacritic、Greek、Cyrillic、絵文字混在）は過度な拡張を避けるため CJK prefix 経路を通さない。絵文字混在トークンは、unicode61 が indexing とクエリの両側で絵文字を削ぐため FTS 層で素の ASCII トークンと区別できず（`foo🎉` は FTS 上 `foo` と等価）、絵文字単独の部分一致も同じ理由で 0 件になる。絵文字の同一性が必要な場合は `exact` を使う。",
                new JsonObject
                {
                    ["type"] = "object",
