    ...
    private JsonNode HandleToolsList(JsonNode? id)
    {
        var tools = new JsonArray
        {
            CreateToolDefinition(
                "search",
                "Full-text search across indexed code chunks using FTS5. Returns compact match-centered snippets with line metadata. CJK tokens (Han, Hiragana, Katakana, Hangul, and halfwidth Katakana) in the literal-safe path are auto-upgraded to FTS5 prefix phrases so `search 計算` matches content containing `計算する` without needing `rawQuery`. Non-CJK tokens — including Latin-diacritic (`naïve`), Greek, Cyrillic, and emoji-mixed text — keep exact-phrase semantics to avoid over-widening; emoji substring search is additionally limited by FTS5's default unicode61 tokenizer categories, which drop symbol codepoints entirely. / FTS5を使ったコードチャンクの全文検索。一致中心の軽量スニペットと行メタデータを返す。literal-safe 経路では CJK トークン（漢字・ひらがな・カタカナ・ハングル・半角カナ）のみ自動で FTS5 prefix phrase に昇格するため、`search 計算` は `rawQuery` なしでも `計算する` を含むコードにマッチする。CJK 以外の非 ASCII トークン（`naïve` 等の Latin-diacritic、Greek、Cyrillic、絵文字混在）は過度な拡張を避けるため完全一致のまま。絵文字部分一致は FTS5 既定 unicode61 の category 設定で記号コードポイントが全て破棄されるため、さらに強い制限がある。",
                new JsonObject
                {
                    ["type"] = "object",
