...
public partial class DbReader
{
    ...
    public int CountSearchSymbols(IReadOnlyList<string>? queries, int limit = 20, string? kind = null, string? lang = null, IReadOnlyList<string>? pathPatterns = null, IReadOnlyList<string>? excludePathPatterns = null, bool excludeTests = false, DateTime? since = null, bool exact = false)
    {
        ...
        if (validQueries != null && validQueries.Count == 1)
        {
            ...
            innerSql += exact
                ? _foldReady
                    ? allowLeafFallback
                        ? " AND (s.name_folded = @query0 OR (f.lang = 'sql' AND ((sql_segment_count(s.name) = @query0SegmentCount AND sql_normalize_name_folded(s.name) = @query0NormalizedFolded) OR sql_leaf_name(s.name) = @query0Leaf COLLATE NOCASE)))"
        ...
        if (effectiveQueries != null && effectiveQueries.Count > 0)
        {
            var orClauses = exact
                ? string.Join(" OR ", effectiveQueries.Select((queryValue, idx) =>
                {
                    ...
                    return _foldReady
                        ? allowLeafFallback
                            ? $"(s.name_folded = @query{idx} OR (f.lang = 'sql' AND ((sql_segment_count(s.name) = @query{idx}SegmentCount AND sql_normalize_name_folded(s.name) = @query{idx}NormalizedFolded) OR sql_leaf_name(s.name) = @query{idx}Leaf COLLATE NOCASE)))"
    ...
    public QueryCountResult CountDefinitionsTotal(string query, string? kind = null, string? lang = null, IReadOnlyList<string>? pathPatterns = null, IReadOnlyList<string>? excludePathPatterns = null, bool excludeTests = false, DateTime? since = null, bool exact = false)
    {
        ...
        if (!string.IsNullOrWhiteSpace(query))
        {
            ...
            sql += exact
                ? _foldReady
                    ? allowLeafFallback
                        ? " AND (s.name_folded = @query OR (f.lang = 'sql' AND ((sql_segment_count(s.name) = @querySegmentCount AND sql_normalize_name_folded(s.name) = @queryNormalizedFolded) OR sql_leaf_name(s.name) = @queryLeaf COLLATE NOCASE)))"
        ...
        bool preferNonEnumMember)
    {
        ...
        var nameCondition = _foldReady
            ? allowLeafFallback
                ? "(s.name_folded = @queryFolded OR (f.lang = 'sql' AND ((sql_segment_count(s.name) = @querySegmentCount AND sql_normalize_name_folded(s.name) = @queryNormalizedFolded) OR sql_leaf_name(s.name) = @queryLeaf COLLATE NOCASE)))"
