...
| `callers` | List callers for a named symbol in supported languages; `kind` filters by reference kind |
| `callees` | List callees for a named symbol in supported languages; identical constructor `call` + `instantiate` rows collapse by default |
...
| `callers` | 対応言語で指定シンボルの caller を列挙。`kind` は reference kind を指す |
| `callees` | 対応言語で指定シンボルの callee を列挙。constructor site の `call` + `instantiate` 重複は既定で集約 |
