...
CodeIndex is designed for mixed-language codebases. A project containing `.cs`, `.py`, `.js`, `.sql`, `.yml`, and other files will be indexed in a single pass — every file type in the table below is included automatically. You can search across all languages at once, or filter by a specific language using `WHERE lang = 'csharp'` etc.

#### Supported languages

All listed languages are **fully indexed** (file metadata, content chunks, FTS5 full-text search). Languages marked with **Symbols** also get regex-based extraction of function/class/import names.
...
Languages without symbol extraction are still fully searchable via FTS5 — only the structured symbol query (`WHERE kind = 'function'`) is unavailable for those languages.
...
CodeIndexは多言語が混在するコードベースを想定して設計されています。`.cs`、`.py`、`.js`、`.sql`、`.yml` などが混在するプロジェクトでも1回のインデックスですべて対応します。全言語を横断して検索することも、`WHERE lang = 'csharp'` のように特定言語に絞り込むことも可能です。

#### 対応言語一覧

下記の全言語がファイルメタデータ、コンテンツチャンク、FTS5全文検索に**完全対応**しています。**シンボル**列が✅の言語は、関数・クラス・インポート名の正規表現抽出にも対応しています。
...
シンボル抽出非対応の言語でもFTS5による全文検索は問題なく利用できます。利用できないのは構造化シンボルクエリ（`WHERE kind = 'function'` など）のみです。
