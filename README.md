...
- Python、JavaScript/TypeScript、C#、Go、Rust、Java、Kotlin、Ruby、C/C++、PHP、Swift、Dart、Scala、Elixir、Lua、VB.NET、SQL の call graph 系調査では `references`、`callers`、`callees` を使う（19 言語）。SQL は T-SQL / MySQL / MariaDB の `EXEC` / `EXECUTE` / `CALL` を括弧あり／なしの両形で拾う。F# は空白区切りの呼び出し構文を使うため graph query は未対応で、代わりに `search` を使う。名前がすでに確定している場合は `--exact` を付けると `Run` が `RunAsync` / `RunImpact` に広がらず、`definition` も同様に `--exact` をサポートする。C# の enum member は symbol として索引される一方、`Nested.A` のような enum-member access edge はまだ索引していないため、active scope にその enum member 候補が含まれる場合、`inspect` は callable 候補があれば `graph_supported` を維持したまま `graph_degraded=true` / `unsupported_symbol_kind=enum_member` を返し、exact `references` / `callers` / `callees` も zero/count payload と成功した JSON row の両方で同じ制約を返す。
