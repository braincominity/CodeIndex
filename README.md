...
![License](https://img.shields.io/badge/License-PolyForm%20Perimeter%201.0.0-orange)
...
- **License**: [PolyForm Perimeter 1.0.0](LICENSE); competing products or services require a separate agreement
...
CodeIndex and the `cdidx` binaries published from this repository are
source-available under the [PolyForm Perimeter License 1.0.0](LICENSE). You
may use, modify, and distribute the software, but you may not provide a product
or service that competes with CodeIndex without a separate written agreement
with Widthdom.

Internal company use for searching your own code is not the restricted case.
Selling, hosting, repackaging, renaming, or publishing a fork as a competing
AI-code-search, MCP retrieval, editor extension, SaaS, container image, or
package is restricted. See [COMMERCIAL_LICENSE.md](COMMERCIAL_LICENSE.md) and
[TRADEMARKS.md](TRADEMARKS.md).

This change is not intended to retroactively rewrite copies that were already
received under a previous license. Treat historical tags, branches, packages,
and archives as governed by the license that shipped with that specific copy;
current and future official releases are governed by the license included with
those releases.
...
| `--max-line-width <n>` | `search`, `references`, `find`, `excerpt`, `inspect` | Clamp very long single-line snippet/reference/excerpt payloads around the relevant match (default: 512, max: 4096; `0` disables truncation) |
...
- Add `--snippet-lines <n>` to `search` when you need tighter JSON output before handing results to another model or tool. For minified / transpiled / generated files whose matches sit on a single huge physical line, also pass `--max-line-width <n>` so each snippet line is clamped around the first match token with `...(+N)...` markers, or use `--max-line-width 0` to disable truncation entirely (default: 512, max: 4096).
...
- **ライセンス**: [PolyForm Perimeter 1.0.0](LICENSE)。競合する製品・サービスには別途契約が必要です
...
このリポジトリから公開される CodeIndex と `cdidx` バイナリは、
[PolyForm Perimeter License 1.0.0](LICENSE) の下で source-available として
提供されます。ソフトウェアの利用・改変・配布は可能ですが、CodeIndex と
競合する製品またはサービスを第三者に提供するには、Widthdom との別途の書面
契約が必要です。

自社コードを検索するための社内利用は、ここで制限している対象ではありません。
一方で、競合する AI コード検索、MCP retrieval、エディタ拡張、SaaS、コンテナ
イメージ、パッケージとして Fork を販売・ホスト・再パッケージ・改名・公開する
ことは制限されます。詳細は [COMMERCIAL_LICENSE.md](COMMERCIAL_LICENSE.md) と
[TRADEMARKS.md](TRADEMARKS.md) を参照してください。

この変更は、過去に別ライセンスで既に取得されたコピーを遡及的に書き換える
ものではありません。過去の tag、branch、package、archive は、そのコピーに
同梱されていたライセンスに従うものとして扱ってください。現在および今後の
公式リリースには、それぞれのリリースに同梱されたライセンスが適用されます。
...
cdidx search "auth*" --fts               # 生のFTS5構文（前方一致検索）。壊れた入力はヒント付きの使用エラーになる
...
`search --json` と MCP の `search` は、チャンク全文ではなく一致中心の軽量スニペットを返します。各結果には `chunk_start_line`、`chunk_end_line`、`snippet_start_line`、`snippet_end_line`、`snippet`、`match_lines`、`highlights`、`context_before`、`context_after`、`truncated_line_count` が含まれます。抜粋の長さは `--snippet-lines <n>` で調整でき（デフォルト: 8、最大: 20）、minified / transpiled で 1 行が極端に長いファイルでは `--max-line-width <n>` を使って各行を最初の一致トークン周辺へクランプできます（デフォルト: 512、最大: 4096、`0` で切り詰め解除）。クランプされた行はスニペット内に `...(+N)...` マーカーが入り、JSON / MCP 出力では `highlights[].truncated` / `highlights[].original_line_length` でも検出できます。
...
| `--max-line-width <n>` | `search`, `references`, `find`, `excerpt`, `inspect` | 極端に長い1行のスニペット・参照文脈・抜粋を、関連箇所の周辺だけに切り詰める（デフォルト: 512、最大: 4096、`0` で切り詰め解除） |
...
- Python、JavaScript/TypeScript、C#、Go、Rust、Java、Kotlin、Ruby、C/C++、PHP、Swift、Dart、Scala、Elixir、Lua、VB.NET の call graph 系調査では `references`、`callers`、`callees` を使う（18 言語）。JavaScript/TypeScript では `new Foo;`、`new Date;`、`new Demo.Provider;`、`new Box<number>;` のような `()` 省略 zero-arg `new` も `instantiate` edge として索引される。F# は空白区切りの呼び出し構文を使うため graph query は未対応で、代わりに `search` を使う。SQL の `references` / `callers` / `callees` / `impact` は、索引対象になった T-SQL / MySQL / MariaDB の `EXEC` / `EXECUTE` / `CALL` edge に対して機能し、対応している `CREATE PROCEDURE` / `CREATE FUNCTION` body から呼び出し元コンテナも復元する。ただし parser ベースではなく regex ベースの抽出なので、dynamic SQL、未対応の方言固有構文、索引済み edge 形に乗らない呼び出しまで広く洗いたいときは `search` を fallback に使う。名前がすでに確定している場合は `--exact` を付けると `Run` が `RunAsync` / `RunImpact` に広がらず、`definition` も同様に `--exact` をサポートする。SQL の exact / non-exact graph reader は source 側が qualified なら schema/package-qualified identity を維持し、exact lookup では `"sales.fn_Target"` のようなドット入り quoted single identifier も `sales.fn_Target` のような本物の qualified name と混同しない。C# の qualified enum-member access（`Nested.A` や `Outer.First.None` など）は索引され、その参照行は symbol extraction が回収できる最も狭い owner に紐づく。
...
- `unused` で潜在的なデッドコードを検出する — 定義されているが一度も参照されていないシンボル（グラフ対応言語でのみ有効）。結果は bucket 化され、private/file-local な候補が public/exported や config/reflection suspect より前に維持される一方、返却ページは `--limit` 適用前に bounded overfetch と bucket 間の分散を行うため、ノイズの多い 1 bucket が他を隠しにくく、大きい `--limit` も暗黙に切り詰めない。suspect bucket は public/exported な property のうち、config バインドされていそうなもの、または C# で近傍に serializer/ORM 系 attribute があるもの向けに予約されている。未対応言語を `--lang` で指定した場合は偽の unused hit を返さず、`graph_supported=false` 付きの空ページを返す。CLI JSON ではシンボルごとに `unused_bucket` / `unused_confidence` / `unused_reason`、MCP では `unusedBucket` / `unusedConfidence` / `unusedReason` を返し、どちらも返却ページ単位の `returned_bucket_counts` を含む。ゼロ件でも JSON schema は同じで、`count`、空の `symbols`、空の page count、graph trust metadata を返し、`unused --json` はその payload を exit code `0` で返す。qualified な C# enum-member access edge は索引されるため、`unused` は参照済み enum member を通常どおり除外しつつ、`Green` のような genuinely unused な enum member を特別な degraded metadata なしで返す。
