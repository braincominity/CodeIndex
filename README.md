...
| `--path <pattern>` | `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files` | Restrict results to paths containing this text |
| `--exclude-path <pattern>` | `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files` | Exclude paths containing this text (repeatable) |
| `--exclude-tests` | `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files` | Exclude likely test files and prefer production code |
...
| `--path <pattern>` | `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files` | 指定文字列を含むパスに結果を絞る |
| `--exclude-path <pattern>` | `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files` | 指定文字列を含むパスを除外（繰り返し指定可） |
| `--exclude-tests` | `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files` | テストらしいパスを除外し、本番コードを優先 |
...
cdidxはAI対応のコード検索ツールとして設計されています。すべてのクエリコマンドは `--json` でJSONライン出力に対応し、プログラムからのパースが容易です。`search`、`definition`、`references`、`callers`、`callees`、`symbols`、`files` は `--path`、繰り返し指定できる `--exclude-path`、`--exclude-tests` で共通に絞り込みできるため、AIクライアントは抜粋取得前にノイズを減らせます。`search --json` と MCP の `search` は、チャンク全文ではなく一致中心の軽量スニペットと明示的な行メタデータを返し、`--snippet-lines` でそのサイズを先に制限できます。シンボルJSONにはより豊富な定義メタデータが含まれ、古いDBを新しいcdidxで開いた場合も、可能なら不足するシンボル列を自動追加し、新しい参照テーブルも作成します。DBをその場で移行できない場合でも、シンボル読み取りは旧レイアウトへフォールバックし、クラッシュしません。
