...
- Use `symbols` for named code entities in symbol-aware languages: Python, JavaScript/TypeScript, C#, Go, Rust, Java, Kotlin, Ruby, C/C++, PHP, Swift, Dart, Scala, Elixir, and Lua.
- Use `search` for raw text, comments, strings, or languages without structured symbol extraction such as R, Shell, SQL, Markdown, YAML, JSON, TOML, HTML/CSS, Vue, Svelte, and Terraform.
...
- Python、JavaScript/TypeScript、C#、Go、Rust、Java、Kotlin、Ruby、C/C++、PHP、Swift、Dart、Scala、Elixir、Lua のようなシンボル抽出対応言語では、名前ベースの調査に `symbols` を使う。
- R、Shell、SQL、Markdown、YAML、JSON、TOML、HTML/CSS、Vue、Svelte、Terraform のような構造化シンボル抽出が弱い言語や、コメント・文字列・生テキストを探す場合は `search` を使う。
