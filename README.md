...
|---|---|---|
| `--db <path>` | All commands | Database file path. `index` defaults to `<projectPath>/.cdidx/codeindex.db`; query commands default to `.cdidx/codeindex.db` in the current directory. For explicit query DBs outside `<project>/.cdidx/`, workspace metadata such as `project_root`, `git_head`, and `git_is_dirty` comes from the persisted `indexed_project_root` stored in that DB, not from the caller's current working directory. Legacy explicit DBs created before that metadata existed may return those fields as `null` / absent until you rerun `cdidx index <projectPath> --db <path>` or a scoped update that actually commits at least one file delete/update against the intended project. Project-local `<project>/.cdidx/codeindex.db` keeps using the `.cdidx` path itself as the source of truth, so moving or renaming the repo does not leave stale workspace metadata behind. |
...
| オプション | 対象 | 説明 |
|---|---|---|
| `--db <path>` | 全コマンド | DBファイルパス。`index` のデフォルトは `<projectPath>/.cdidx/codeindex.db`、クエリ系コマンドのデフォルトはカレントディレクトリの `.cdidx/codeindex.db`。`<project>/.cdidx/` の外にある明示指定 query DB の `project_root`、`git_head`、`git_is_dirty` などの workspace metadata は、呼び出し元のカレントディレクトリではなく、その DB に保存された `indexed_project_root` から解決される。保存前の古い explicit DB では、意図した project に対して `cdidx index <projectPath> --db <path>`、または少なくとも 1 件の file delete/update を実際に commit する scoped update を一度実行するまで、これらの項目が `null` / 未出力になることがある。project-local の `<project>/.cdidx/codeindex.db` は `.cdidx` の置き場所自体を正とするため、repo を move/rename しても古い絶対パスを引きずらない。 |
