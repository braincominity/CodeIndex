...
## Install cdidx (if not already installed)

```bash
dotnet tool install -g cdidx
```

Requires .NET 8+ SDK. After install, `cdidx` is available as a global command.
...
## cdidxのインストール（未インストールの場合）

```bash
dotnet tool install -g cdidx
```

.NET 8+ SDKが必要です。インストール後、`cdidx` がグローバルコマンドとして利用可能になります。
