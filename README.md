...
| `--count` | `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files`, `find`, `impact`, `unused`, `hotspots` | Return only the total result count, ignoring `--limit` (with `--json`: a single count object; commands that expose file counts add `files`) |
...
- Use `--count` to get authoritative total counts before fetching full data; it ignores `--limit` (saves tokens for AI agents).
...
| `--count` | `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files`, `find`, `impact`, `unused`, `hotspots` | `--limit` を無視した総件数だけを返す（`--json` 併用時は単一の count オブジェクト。files 件数を出すコマンドは `files` も返す） |
...
- `--count` で `--limit` を無視した総件数を先に確認し、全データ取得前にトークンを節約。
