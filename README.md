...
cdidx is designed as an AI-friendly code search tool. All query commands support `--json` for JSON lines output, making them easy to parse programmatically. `search`, `definition`, `references`, `callers`, `callees`, `symbols`, `files`, and `map` share path-aware narrowing via `--path`, repeatable `--exclude-path`, and `--exclude-tests`, so AI clients can cut noise before fetching excerpts. `map` gives a repo-level overview for the first 30 seconds of investigation, and `search --json` plus MCP `search` emit compact match-centered snippets with explicit line metadata instead of whole chunks. `--snippet-lines` lets callers cap snippet size up front. Symbol JSON now carries richer definition metadata, and opening an older database with a newer cdidx version will auto-add missing symbol columns and create newer reference tables when possible. If the DB cannot be migrated in place, symbol reads fall back to the legacy layout instead of crashing.
...
cdidxはAI対応のコード検索ツールとして設計されています。すべてのクエリコマンドは `--json` でJSONライン出力に対応し、プログラムからのパースが容易です。`search`、`definition`、`references`、`callers`、`callees`、`symbols`、`files`、`map` は `--path`、繰り返し指定できる `--exclude-path`、`--exclude-tests` で共通に絞り込みできるため、AIクライアントは抜粋取得前にノイズを減らせます。`map` は調査開始直後の俯瞰を返し、`search --json` と MCP の `search` は、チャンク全文ではなく一致中心の軽量スニペットと明示的な行メタデータを返し、`--snippet-lines` でそのサイズを先に制限できます。シンボルJSONにはより豊富な定義メタデータが含まれ、古いDBを新しいcdidxで開いた場合も、可能なら不足するシンボル列を自動追加し、新しい参照テーブルも作成します。DBをその場で移行できない場合でも、シンボル読み取りは旧レイアウトへフォールバックし、クラッシュしません。
