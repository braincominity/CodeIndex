...
- If the user has configured a GitHub token (`CDIDX_GITHUB_TOKEN` or `GITHUB_TOKEN`), the suggestion is also filed as a GitHub Issue. No token, no network request.
...
**What is never sent:** source code, file paths, database content, or any data from the indexed project. The description and context fields are validated by [SourceCodeDetector](src/CodeIndex/Cli/SourceCodeDetector.cs) to reject text that looks like pasted code. For implementation details, see the [Developer Guide](DEVELOPER_GUIDE.md#ai-feedback-implementation).
...
- Zig、XAML、Markdown、YAML、JSON、TOML、HTML/CSS、Vue、Svelte のような構造化シンボル抽出が弱い言語や、コメント・文字列・生テキストを探す場合は `search` を使う。
...
- ユーザーが GitHub トークン（`CDIDX_GITHUB_TOKEN` または `GITHUB_TOKEN`）を設定している場合のみ、GitHub Issue としても報告されます。トークンがなければネットワークリクエストは発生しません。
...
**送信されないもの:** ソースコード、ファイルパス、データベース内容、インデックス済みプロジェクトのあらゆるデータ。description と context フィールドは [SourceCodeDetector](src/CodeIndex/Cli/SourceCodeDetector.cs) によりコピペされたコードに見えるテキストを拒否するよう検証されます。実装の詳細は[開発者ガイド](DEVELOPER_GUIDE.md#aiフィードバックの実装)を参照してください。
