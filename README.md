...
| `--exact` | `search`, `symbols`, `definition`, `references`, `callers`, `callees` | `search`: case-sensitive exact substring (no FTS5). Symbol/graph commands: ASCII-case-insensitive exact name match via SQLite `COLLATE NOCASE` (non-ASCII casing not folded). Pairs cleanly with a resolved candidate list from a prior `map` / `inspect` / `search` call so `Run` no longer also pulls in `RunAsync`, `RunImpact`, etc. |
...
| `--exact` | `search`, `symbols`, `definition`, `references`, `callers`, `callees` | `search`: 大文字小文字を区別する完全部分一致（FTS5 バイパス）。symbol / graph 系コマンド: SQLite `COLLATE NOCASE` による ASCII のみの大文字小文字無視完全一致（非 ASCII の casing は畳み込まれない）。`map` / `inspect` / `search` 等で解決済みの候補名をそのまま渡せば `Run` が `RunAsync` / `RunImpact` 等に広がらない。 |
