...
`search`, `definition`, `symbols`, and `files` also share `--path <pattern>`, repeatable `--exclude-path <pattern>`, and `--exclude-tests` filters. Search results prefer source files over tests and docs, and `search` boosts files whose symbol names or paths match the query exactly.
...
| `--path <pattern>` | `search`, `definition`, `symbols`, `files` | Restrict results to paths containing this text |
| `--exclude-path <pattern>` | `search`, `definition`, `symbols`, `files` | Exclude paths containing this text (repeatable) |
| `--exclude-tests` | `search`, `definition`, `symbols`, `files` | Exclude likely test files and prefer production code |
...
cdidx is designed as an AI-friendly code search tool. All query commands support `--json` for JSON lines output, making them easy to parse programmatically. `search`, `definition`, `references`, `callers`, `callees`, `symbols`, and `files` share path-aware narrowing via `--path`, repeatable `--exclude-path`, and `--exclude-tests`, so AI clients can cut noise before fetching excerpts. Symbol JSON now carries richer definition metadata, and opening an older database with a newer cdidx version will auto-add missing symbol columns and create newer reference tables when possible. If the DB cannot be migrated in place, symbol reads fall back to the legacy layout instead of crashing.
...
```bash
cdidx search "keyword" --path src/ --exclude-tests
...
`search`、`definition`、`symbols`、`files` は共通で `--path <pattern>`、繰り返し指定できる `--exclude-path <pattern>`、`--exclude-tests` に対応しています。検索結果は tests や docs より source を優先し、`search` はシンボル名やパスがクエリと正確に一致するファイルを上に出します。
...
| `--path <pattern>` | `search`, `definition`, `symbols`, `files` | 指定文字列を含むパスに結果を絞る |
| `--exclude-path <pattern>` | `search`, `definition`, `symbols`, `files` | 指定文字列を含むパスを除外（繰り返し指定可） |
| `--exclude-tests` | `search`, `definition`, `symbols`, `files` | テストらしいパスを除外し、本番コードを優先 |
...
cdidxはAI対応のコード検索ツールとして設計されています。すべてのクエリコマンドは `--json` でJSONライン出力に対応し、プログラムからのパースが容易です。`search`、`definition`、`references`、`callers`、`callees`、`symbols`、`files` は `--path`、繰り返し指定できる `--exclude-path`、`--exclude-tests` で共通に絞り込みできるため、AIクライアントは抜粋取得前にノイズを減らせます。シンボルJSONにはより豊富な定義メタデータが含まれ、古いDBを新しいcdidxで開いた場合も、可能なら不足するシンボル列を自動追加し、新しい参照テーブルも作成します。DBをその場で移行できない場合でも、シンボル読み取りは旧レイアウトへフォールバックし、クラッシュしません。
