...
- Use `impact` for pre-edit ripple checks on callable symbols. When the query resolves to a single class / struct / interface definition and no symbol-level callers exist, `impact` / MCP `impact_analysis` now return reverse file dependencies instead of a silent false negative; when a query resolves to multiple definition files (for example partial classes), the zero-result payload explains that and suggests `deps --path <definition-path> --reverse` or a more specific member query.
...
| `impact_analysis` | Compute transitive callers of a symbol, with single-type fallback to reverse file dependencies and partial-definition hints |
...
- 変更前の波及確認には `impact` を使う。query が単一の class / struct / interface 定義に解決され、symbol-level caller が無い場合は、`impact` / MCP `impact_analysis` が無言の 0 件ではなく reverse file dependency を返す。partial class などで複数定義に解決された場合は、0 件 payload にその旨と `deps --path <definition-path> --reverse` またはより具体的な member query を試すヒントが入る。
...
| `impact_analysis` | シンボルの推移的 caller を算出し、単一定義の型は reverse file dependency にフォールバックし、複数定義時はヒントも返す |
