...
- Start with `status --check --json` when freshness matters, then `map` / `map --json` when you need a quick overview of languages, modules, likely entrypoints, and hot spots before issuing symbol or text queries. Use `index_matches_workspace` and the `workspace_check` mismatch counts to decide whether you need to re-run `cdidx .` before trusting results. If you only have plain `status --json`, use `indexed_at`, `latest_modified`, `git_head`, and `git_is_dirty` as a weaker timestamp/git hint. If you already started with `map --json`, treat `indexed_at` / `latest_modified` there as filter-scoped freshness and `workspace_indexed_at` / `workspace_latest_modified` as the whole-workspace view. For explicit `--db` queries, remember that `project_root` / `git_*` depend on the DB's persisted `indexed_project_root`; a legacy explicit DB may legitimately return those fields as `null` / absent until you rerun indexing against the intended project, or run a scoped update that actually commits a file delete/update, even if the explicit path itself looks like `.../.cdidx/codeindex.db`.
- Use `inspect` when you already have a candidate symbol name and want bundled definition/caller/callee/reference context in one round-trip. Add `--exact` to match leaf-command precision — `exact` is propagated into every bundled sub-query so `inspect Run --exact` will not drag in `RunAsync` / `RunImpact`. `inspect --json` also carries `workspace_indexed_at`, `workspace_latest_modified`, `project_root`, `git_head`, and `git_is_dirty` for trust decisions, and it mirrors `sql_graph_contract_ready` / `sql_graph_contract_degraded_reason` only when the bundled graph sections actually use SQL-backed reads. For legacy explicit DBs, those workspace fields can stay `null` / absent until you rerun indexing against the intended project, or a scoped update actually rewrites/deletes at least one indexed file.
...
- Markdown files now index ATX/setext headings as `heading` symbols, and local anchor references surface as `reference` symbols, so `symbols` and `outline` can navigate document structure directly.
- Use `definition` when you need the declaration text for a named symbol, and add `--body` when the implementation body matters.
  - Use `references`, `callers`, and `callees` for symbol-aware graph questions in graph-supported language filters. The canonical set is exposed by `cdidx languages`; in this release it is Assembly, Batch, C, COBOL, C++, C#, CSS, Dart, Dockerfile, Elixir, F#, Fortran, Go, Gradle, Haskell, Java, JavaScript, Kotlin, Lua, Makefile, Objective-C, Pascal, Perl, PHP, PowerShell, Protobuf, Python, R, Razor/Blazor (`razor`, `blazor`, `cshtml` aliases), Ruby, Rust, Scala, Shell, Smalltalk, SQL, Svelte, Swift, Terraform, TypeScript, VB.NET, Vue, and Zig (44 filters). C/C++, Dart, Go, Elixir, Lua, Haskell, VB.NET, Razor/Blazor, Fortran, Pascal, Objective-C, and Smalltalk include language-specific type/import/call/message/component edges rather than only bare `name(...)` calls. In Ruby, command-syntax calls such as `puts "hi"`, `greet bob`, `include Shared`, `attr_accessor :name`, `before_action :authenticate`, and `raise ArgumentError, "bad"` are also indexed, so the idiomatic no-parens form no longer disappears from the graph. In JavaScript/TypeScript, graph extraction also indexes zero-argument `new` sites that omit `()`, so `new Foo;`, `new Date;`, qualified targets like `new Demo.Provider;`, and one-level generic TypeScript forms like `new Box<number>;` now contribute `instantiate` edges. Terraform also indexes dotted `var.*`, `local.*`, `module.*`, and `data.*` references, plus same-file resource-like `TYPE.NAME` references such as `aws_instance.web` and `depends_on = [aws_s3_bucket.foo]`. F# now indexes parenthesized, pipeline, and common space-separated application call sites such as `printfn "x"` and `List.map increment numbers`. COBOL now indexes common procedural statements as references too, including `CALL`, `COPY`, `GO TO` / `GOTO`, `READ`, `WRITE`, `REWRITE`, `DELETE`, `CLOSE`, `OPEN`, `SEARCH`, `SORT`, `MERGE`, `SET`, `MOVE`, `ADD`, `SUBTRACT`, `MULTIPLY`, `DIVIDE`, `COMPUTE`, `STRING`, `UNSTRING`, `DISPLAY`, `ACCEPT`, and `INSPECT`, so the names involved in everyday control-flow and data-flow statements stay searchable instead of falling back to raw text only. Assembly indexes direct call and branch targets such as `call`, `jmp`, `j*`, `bl` / `blx`, `b`, `b.<cond>`, known conditional branch mnemonics, and `loop`-family mnemonics as graph references. Shell indexes bare function calls in command syntax, so same-file function names remain visible in the graph. PHP import extraction also keeps prefixed `require` / `include` literals such as `__DIR__ . '/bootstrap.php'` searchable as import symbols. Rust grouped `use` trees now keep their path prefixes too, so `use std::collections::{HashMap, HashSet}` indexes both the full path and the leaf name. SQL `references`, `callers`, `callees`, and `impact` work for indexed T-SQL / MySQL / MariaDB `EXEC` / `EXECUTE` / `CALL` stored-procedure edges, including caller-side container attribution for supported `CREATE PROCEDURE` / `CREATE FUNCTION` bodies. They are still regex-based rather than parser-authoritative, so prefer `search` as the broader fallback for dynamic SQL, unsupported dialect-specific constructs, or cases where a SQL-heavy codebase might hide the call outside the indexed edge shapes. Add `--exact` when you already have a precise name (e.g. resolved from `symbols --exact` / `inspect`) so `references Run` no longer pulls in `RunAsync` / `RunImpact` / etc. — same for `callers` / `callees` / `definition`. SQL exact and non-exact graph readers preserve schema/package-qualified identities when the source reference is qualified, while exact SQL lookups keep quoted single identifiers containing dots, such as `"sales.fn_Target"`, distinct from real qualified names such as `sales.fn_Target`. C# qualified enum-member accesses such as `Nested.A` and `Outer.First.None` are indexed, and their reference rows inherit the narrowest enclosing owner that symbol extraction can recover (for example a property or method instead of only the enclosing class).
- F# also indexes common space-separated application call sites such as `printfn "x"` and `List.map increment numbers`, so those forms no longer need to fall back to `search`.
  SQL symbol extraction also keeps optional whitespace around qualified-name dots for the supported DDL patterns, so `sales . seq_orders`, `[sales] . [syn_demo]`, and `"sales" . "ext_demo"` stay indexed as full qualified names instead of collapsing to the first segment.
- Scala block-call forms such as `foreach { ... }`, `Try { ... }`, and `synchronized { ... }` are also indexed, so idiomatic `name { ... }` call sites stay visible in the graph.
...
- Use `symbols` for named code entities in symbol-aware languages (including Python, JavaScript/TypeScript, C#, Go, Rust, Java, Kotlin, Ruby, C/C++, PHP, Swift, Dart, Scala, F#, VB.NET, COBOL, Elixir, Lua, R, Haskell, Shell, SQL, Assembly, Fortran, Objective-C, Pascal, Smalltalk, Terraform, Protobuf, GraphQL, Gradle, Makefile, Dockerfile, Zig, PowerShell, Batch, Markdown, CSS/SCSS). Python also now captures PEP 695 generic functions (`def name[T](...)`) and type aliases (`type Name = ...`) through the same named-symbol flow. C++ nested namespace declarations like `namespace outer::inner {}` now keep the full qualified namespace name, so `symbols --lang cpp --kind namespace --name outer::inner --exact-name` can target them directly. C++ `using` type aliases such as `using Alias = std::vector<int>;`, `template <typename T> using Ptr = T*;`, and simple `typedef ExistingType Alias;` aliases are indexed as `import` symbols, so `symbols --lang cpp --kind import --name Alias --exact-name` can find them too. Swift `typealias` and `associatedtype` declarations now surface as dedicated symbol kinds, so `--kind typealias` / `--kind associatedtype` filters can separate them from real `import` entries. F# now also surfaces top-level namespaces/modules and `member` declarations in `symbols`, even though its graph queries remain unsupported. Assembly symbols include labels, PROC/MACRO blocks, sections/segments, extern/include/import directives, and constants. Add `--exact` when you already have a precise candidate list (e.g. names from a prior `inspect` / `map` / `search` result) so `Run` no longer expands to `RunAsync` / `RunImpact` — pairs cleanly with repeated `--name`.
  CSS/SCSS symbol extraction now covers `:root`, `--custom-property`, `@font-face` family names, pseudo/attribute selectors, and SCSS `%placeholder`, while still keeping exact-name separation such as `:root` vs `.root` vs `#root`. Literal exact-name queries also work for token-like names such as `cdidx symbols --lang css --name=--accent --exact-name` and `cdidx symbols --lang css --name=%button-base --exact-name`.
  C# plain fields now surface as `property` symbols in `symbols`, `definition`, `outline`, and `inspect`, including declarator lists (`private int _x, _y;`), multi-line headers with `volatile` / `unsafe` / `extern` / `ref readonly` modifiers, parenthesized initializers (`= new(\n    () => 42);`), and object / collection initializers (`= new() {\n    ["a"] = 1\n};`). Function-pointer fields such as `public delegate*<int, void> Callback;` are captured as well. Query them directly with `cdidx symbols --lang csharp --kind property --name Count --exact-name` just like real C# properties.
- Use `outline` to see the full symbol structure of a single file without reading its content.
- Use `search` for raw text, comments, strings, or languages without structured symbol extraction such as XAML, YAML, JSON, TOML, HTML, Vue, Svelte. The default search mode is literal-safe and quotes tokens for you; trailing `*` is treated as a prefix shorthand, and `--fts` is only for raw FTS5 syntax when you intentionally want operators like `NEAR` or `OR`. Use `--exact-substring` for punctuation-heavy literals, operators, or other text where FTS tokenization would be a poor fit.
  - Add `--exclude-tests` unless you are explicitly investigating tests. It is boundary-aware for common test names like `tests/`, `test_*.py`, `*_test.py`, and `__tests__/`, while leaving mid-word filenames such as `latest.py` and `request.py` in the result set.
- Add `--path <text>` and repeatable `--exclude-path <text>` before broad searches so results stay inside the relevant module.
- Add `--snippet-lines <n>` to `search` when you need tighter JSON output before handing results to another model or tool. For minified / transpiled / generated files whose matches sit on a single huge physical line, also pass `--max-line-width <n>` so each snippet line is clamped around the first match token with `...(+N)...` markers (`0` disables clamping; default: 512, max: 4096).
...
- Use `unused` to find potentially dead code — symbols defined but never referenced (only meaningful for graph-supported languages). Results are bucketed so private/file-local candidates stay earlier than public/exported or config/reflection suspects, and returned pages use bounded overfetch plus cross-bucket diversification before `--limit` is applied so one noisy bucket is less likely to hide the rest without silently capping large `--limit` requests. The suspect bucket is reserved for public/exported symbols that look config-bound or, for C#, have nearby serializer/ORM-style attributes on class-like types or properties. Unsupported `--lang` filters now return an empty page with `graph_supported=false` instead of fake unused hits. CLI JSON uses per-symbol `unused_bucket` / `unused_confidence` / `unused_reason`, MCP uses `unusedBucket` / `unusedConfidence` / `unusedReason`, and both include page-scoped `returned_bucket_counts`. Zero-result JSON pages keep the same schema (`count`, empty `symbols`, empty page counts, and graph trust metadata) instead of going silent, and `unused --json` returns that payload with exit code `0` so machine clients can consume it as a normal empty page. Because qualified C# enum-member access edges are indexed, `unused` now reports genuinely unused enum members as normal results while keeping referenced enum members out, without special degraded metadata.
- Use `hotspots` to find central, high-impact symbols. Names that are unique within the active `--lang` / `--kind` candidate set use codebase-wide reference counts; duplicate-name families fall back to conservative same-file counts, and same-file duplicate rows may be grouped when the DB cannot disambiguate targets. Cross-file grouping of duplicate families is trusted only when the index carries the current authoritative hotspot-family stamp; legacy or partially refreshed DBs stay on the conservative path until a full authoritative refresh.
...
- Use `--count` for preflight sizing before fetching full data. `search` / `definition` / `references` / `callers` / `callees` / `symbols` / `files` / `find` / `unused` return authoritative totals, while `impact` and `hotspots` still report visible page counts and respect `--limit` (saves tokens for AI agents).
...
実装の詳細（スキーマ、索引パイプライン、MCP挙動）は [DEVELOPER_GUIDE.md](DEVELOPER_GUIDE.md) を参照してください。
...
- 鮮度が重要な場合は、まず `status --check --json` を見てから、全体像の把握には `map` / `map --json` を使う。`index_matches_workspace` と `workspace_check` の mismatch count を見て再インデックスが必要か判断する。通常の `status --json` しか使わない場合は、`indexed_at`、`latest_modified`、`git_head`、`git_is_dirty` を弱い timestamp / git hint として読む。すでに `map --json` を使っている場合は、その `indexed_at` / `latest_modified` は絞り込み結果に対する値、`workspace_indexed_at` / `workspace_latest_modified` はワークスペース全体に対する値として読む。explicit `--db` query の `project_root` / `git_*` は DB に保存済みの `indexed_project_root` に依存するため、古い explicit DB では intended project に対して indexing を再実行するか、実際に file delete/update を commit する scoped update を流すまで `null` / 未出力が正しい場合があり、明示パス自体が `.../.cdidx/codeindex.db` でも同じ。
- 候補シンボル名が決まっていて、定義・caller・callee・参照をまとめて欲しいときは `inspect` を使う。leaf コマンドと同じ precision にしたいときは `--exact` を付ける（bundle 内の全 sub-query に伝播するため、`inspect Run --exact` が `RunAsync` / `RunImpact` を巻き込むことはない）。`inspect --json` には `workspace_indexed_at`、`workspace_latest_modified`、`project_root`、`git_head`、`git_is_dirty` も含まれ、bundle 内で SQL graph read が実際に使われた場合だけ `sql_graph_contract_ready` / `sql_graph_contract_degraded_reason` も返るため、信頼判断も同時に行える。ただし legacy explicit DB では intended project に対して indexing を再実行するか、実際に file を更新/削除する scoped update が commit されるまでこれらの workspace 項目が `null` / 未出力になりうる。
- `definition`、`references`、`callers`、`callees`、`impact` に入る前に、まず `symbols` で候補名を固める。対象シンボルが分かったら `--exact` / `--exact-name` を付けて、`Run` が `RunAsync` / `RunImpact` まで広がらないようにする。Rust では `crate::foo::Bar` のような path-qualified spelling と末尾 `!` の macro 名をそのまま投げても、`symbols` / `references` / `callers` / `callees` / `inspect` 側で leaf 名へ正規化される。`use std::collections::{HashMap, HashSet}` のような grouped import も、path prefix と leaf 名の両方が索引される。`--lang` に渡す正式名を確認したいときは `languages`、ファイル単位のシンボル構造だけ見たいときは `outline` を使う。
- 名前付きシンボルの宣言を取りたいときは `definition` を使い、本体も必要なら `--body` を付ける。
- graph 対応言語フィルタでの call graph 系調査では `references`、`callers`、`callees` を使う。canonical な対応セットは `cdidx languages` が返し、このリリースでは Assembly、Batch、C、COBOL、C++、C#、CSS、Dart、Dockerfile、Elixir、F#、Go、Gradle、Haskell、Java、JavaScript、Kotlin、Lua、Makefile、Perl、PHP、PowerShell、Protobuf、Python、R、Ruby、Rust、Scala、Shell、SQL、Svelte、Swift、Terraform、TypeScript、VB.NET、Vue、Zig の 37 フィルタ。Ruby では `puts "hi"`、`greet bob`、`include Shared`、`attr_accessor :name`、`before_action :authenticate`、`raise ArgumentError, "bad"` のような command syntax も索引されるため、慣用的な `()` 省略形が graph から消えない。JavaScript/TypeScript では `new Foo;`、`new Date;`、`new Demo.Provider;`、`new Box<number>;` のような `()` 省略 zero-arg `new` も `instantiate` edge として索引される。Terraform では `var.*`、`local.*`、`module.*`、`data.*` の dotted 参照に加えて、`aws_instance.web` や `depends_on = [aws_s3_bucket.foo]` のような同一ファイル内の resource-like `TYPE.NAME` 参照も索引される。Shell は同一ファイル内の function 名をコマンド構文でも索引するため、`setup && cleanup` のような bare call も graph から消えない。Assembly は `call`、`jmp`、`j*`、`bl` / `blx`、`b`、`b.<cond>`、既知の条件分岐 mnemonic、`loop` 系 mnemonic などの直接 call/branch ターゲットを graph 参照として索引する。F# は親付き呼び出し、pipeline 呼び出し、空白区切り application の common な形も graph で拾えるようになった。COBOL では `PERFORM ... THRU ...` が間の paragraph と section をまたいで展開されるため、section-level の graph でも直接の start target だけでなく range 全体が解決される。SQL の `references` / `callers` / `callees` / `impact` は、索引対象になった T-SQL / MySQL / MariaDB の `EXEC` / `EXECUTE` / `CALL` edge に対して機能し、対応している `CREATE PROCEDURE` / `CREATE FUNCTION` body から呼び出し元コンテナも復元する。ただし parser ベースではなく regex ベースの抽出なので、dynamic SQL、未対応の方言固有構文、索引済み edge 形に乗らない呼び出しまで広く洗いたいときは `search` を fallback に使う。名前がすでに確定している場合は `--exact` を付けると `Run` が `RunAsync` / `RunImpact` に広がらず、`definition` も同様に `--exact` をサポートする。SQL の exact / non-exact graph reader は source 側が qualified なら schema/package-qualified identity を維持し、exact lookup では `"sales.fn_Target"` のようなドット入り quoted single identifier も `sales.fn_Target` のような本物の qualified name と混同しない。C# の qualified enum-member access（`Nested.A` や `Outer.First.None` など）は索引され、その参照行は symbol extraction が回収できる最も狭い owner に紐づく。
  SQL の symbol extraction も、対応している DDL パターンでは qualified name の `.` 前後の空白を保持するようになり、`sales . seq_orders`、`[sales] . [syn_demo]`、`"sales" . "ext_demo"` のような名前も先頭セグメントに潰れず完全修飾名のまま索引される。
- Scala の block-call 形式 `foreach { ... }`、`Try { ... }`、`synchronized { ... }` も索引されるため、慣用的な `name { ... }` 呼び出しが graph から消えない。
- 変更前の波及確認には `impact` を使う。active な `--lang` / `--path` / `--exclude-path` / `--exclude-tests` scope と graph-supported language の範囲で、query が単一の class / struct / interface 定義に解決され、symbol-level caller が無い場合は、`impact` / MCP `impact_analysis` が無言の 0 件ではなく heuristic な file-level dependency hint を返すことがある。同名の `namespace` や `import` など non-callable な sibling 定義はこの fallback を妨げず、pure import/namespace query は説明なしの 0 件ではなく `non_callable_symbol_kind` guidance を返す。ノイズを減らすため、hint は「対象型の member 名に一致する参照があり、かつ同じ file 内の indexed symbol metadata（signature や return type など）に対象型の構造化された痕跡がある」ファイルにだけ出し、コメントや文字列の生テキスト一致では昇格しない。この evidence 判定も Unicode-aware なので、全角やアクセント付き識別子でも exact-match と同じ意味論で扱う。ただし現行グラフは各 call の解決先 file/type を保持していないため、その hint は authoritative ではない。一方でこれらの hint も通常の成功出力として返るため、クライアント側では `impact_mode` / `heuristic` / `hint_count` / `truncated` を見て判定する。`count` / `file_count` は返した可視結果の件数を表し、`confirmed_count` / `confirmed_file_count` は symbol-level caller の確定件数を保持する。`impact --json --count` も full payload と同じ `*_count` フィールド名を使う。scoped query が partial class や同一ファイル内の同名 class など複数の class-like 定義に解決された場合は、0 件 payload にその旨と `deps --path <definition-path> --reverse` またはより具体的な member query を試すヒントが入る。
  - Python、JavaScript/TypeScript、C#、Go、Rust、Java、Kotlin、Ruby、C/C++、PHP、Swift、Dart、Scala、F#、VB.NET、Elixir、Lua、R、Haskell、Zig、PowerShell、Batch、CSS/SCSS のようなシンボル抽出対応言語では、名前ベースの調査に `symbols` を使う。C++ の `using Alias = ...;`、`template <typename T> using Ptr = ...;`、`typedef ExistingType Alias;` に加えて `namespace fs = std::filesystem;` や `using namespace std::chrono_literals;` も `import` シンボルとして拾えるため、`cdidx symbols --lang cpp --kind import --name Alias --exact-name` で直接引ける。PHP では prefixed `require` / `include` も `import` シンボルとして保持されるので、`__DIR__ . '/bootstrap.php'` のような定番の bootstrap パスも検索しやすい。Swift の `typealias` と `associatedtype` は専用のシンボル種別として出るため、`--kind typealias` / `--kind associatedtype` で `import` から分離して絞り込める。F# は `symbols` で top-level namespace/module と `member` 宣言も拾えるようになった一方、graph query は引き続き未対応です。`inspect` / `map` / `search` 等で既に候補名が特定できている場合は `--exact` を付けると、`Run` が `RunAsync` / `RunImpact` に広がらず、渡した名前だけが返る（繰り返しの `--name` と併用可）。
  CSS/SCSS の symbol extraction は `:root`、`--custom-property`、`@font-face` の family 名、疑似/属性セレクタ、SCSS `%placeholder` まで対応しており、`:root` と `.root` と `#root` も exact-name で分離して扱える。`cdidx symbols --lang css --name=--accent --exact-name` や `cdidx symbols --lang css --name=%button-base --exact-name` のような token 風の literal 名もそのまま exact-name で引ける。
  C# の通常フィールドは `symbols` / `definition` / `outline` / `inspect` で `property` シンボルとして見えるようになった。`private int _x, _y;` のような declarator list、`volatile` / `unsafe` / `extern` / `ref readonly` 付きの複数行ヘッダ、`= new(\n    () => 42);` のような括弧付き初期化子、`= new() {\n    ["a"] = 1\n};` のようなオブジェクト/コレクション初期化子にも対応する。`public delegate*<int, void> Callback;` のような関数ポインタフィールドも拾える。C++ の nested namespace 宣言 `namespace outer::inner {}` も完全修飾名のまま `symbols` に載るため、`cdidx symbols --lang cpp --kind namespace --name outer::inner --exact-name` で直接引ける。C# の通常プロパティと同様に `cdidx symbols --lang csharp --kind property --name Count --exact-name` で直接引ける。
  - 1ファイルのシンボル構造をファイル内容を読まずに把握したいときは `outline` を使う。
  - Markdown ファイルは ATX / setext 見出しを `heading` シンボルとして索引し、local anchor 参照も `reference` シンボルとして表面化するため、`symbols` と `outline` でドキュメント構造を直接たどれる。
    - XAML、YAML、JSON、TOML、HTML、Vue、Svelte のような構造化シンボル抽出が弱い言語や、コメント・文字列・生テキストを探す場合は `search` を使う。通常の `search` は literal-safe で、FTS 構文を自分で組み立てなくても token を quote してくれる。末尾の `*` は prefix shorthand として扱われ、生の FTS5 構文を使いたいときだけ `--fts` を使う。記号を多く含む literal、演算子、FTS tokenization と相性の悪い文字列には `--exact-substring` を使う。
  - テストを明示的に調べるのでなければ、まず `--exclude-tests` を付ける。`tests/`、`test_*.py`、`*_test.py`、`__tests__/` のような典型的な test パスだけを除外し、`latest.py` や `request.py` のような mid-word 名は落とさない。
- 広い検索を始める前に `--path <text>` と繰り返し指定できる `--exclude-path <text>` を使って対象モジュールに絞る。
- 別のモデルやツールへ渡す前提で検索JSONをさらに細くしたいときは、`search` に `--snippet-lines <n>` を付ける。minified / transpiled / 生成ファイルのように 1 物理行が極端に長い対象では、合わせて `--max-line-width <n>` を指定するとスニペットの各行が最初のマッチトークン周辺でクランプされ、`...(+N)...` マーカーが入る（`0` でクランプ解除、デフォルト: 512、最大: 4096）。
...
- `unused` で潜在的なデッドコードを検出する — 定義されているが一度も参照されていないシンボル（グラフ対応言語でのみ有効）。結果は bucket 化され、private/file-local な候補が public/exported や config/reflection suspect より前に維持される一方、返却ページは `--limit` 適用前に bounded overfetch と bucket 間の分散を行うため、ノイズの多い 1 bucket が他を隠しにくく、大きい `--limit` も暗黙に切り詰めない。suspect bucket は public/exported な symbol のうち、config バインドされていそうなもの、または C# で class/type や property に serializer/ORM 系 attribute があるもの向けに予約されている。未対応言語を `--lang` で指定した場合は偽の unused hit を返さず、`graph_supported=false` 付きの空ページを返す。CLI JSON ではシンボルごとに `unused_bucket` / `unused_confidence` / `unused_reason`、MCP では `unusedBucket` / `unusedConfidence` / `unusedReason` を返し、どちらも返却ページ単位の `returned_bucket_counts` を含む。ゼロ件でも JSON schema は同じで、`count`、空の `symbols`、空の page count、graph trust metadata を返し、`unused --json` はその payload を exit code `0` で返す。qualified な C# enum-member access edge は索引されるため、`unused` は参照済み enum member を通常どおり除外しつつ、`Green` のような genuinely unused な enum member を特別な degraded metadata なしで返す。
- `hotspots` で中心的かつ影響の大きいシンボルを特定する。active な `--lang` / `--kind` 候補集合で一意な名前は codebase 全体の参照件数を使い、同名ファミリーは保守的な same-file 件数へフォールバックし、DB が対象を曖昧なく結べない同一ファイル重複行は集約される。duplicate family の cross-file 集約は、current の authoritative hotspot-family stamp を持つ index でのみ信頼し、legacy DB や部分更新直後の未 stamp DB は full refresh まで保守経路に留まる。
- `files --since <datetime>` や `search --since <datetime>` で最近変更されたコードに絞る。
- `index` の `--dry-run` でDBに書き込まずインデックス対象を事前確認。
- `--count` で取得件数の見積もりを先に行う。`search` / `definition` / `references` / `callers` / `callees` / `symbols` / `files` / `find` / `unused` は総件数、`impact` と `hotspots` は visible page count を返すため、必要に応じて `--limit` の影響を考慮する。
