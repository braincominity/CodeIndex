...
`status` summary freshness is based on the stored `indexed_at` and `latest_modified` timestamps, not on how long ago the index was built. A clean workspace with `indexed_at >= latest_modified` should read as fresh even if the index itself is older than a few minutes.
