...
| Search | `find` | Literal substring search inside one known indexed file; use `--snippet-lines` / `snippetLines` to widen match context | `find_in_file` |
...
| Search | `find` | 既知の indexed file 内で literal substring 検索。`--snippet-lines` / `snippetLines` で一致前後の文脈行数を調整 | `find_in_file` |
