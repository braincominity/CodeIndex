...
- **Symlinks are skipped during scan** — `FileIndexer.EnumerateDirectory` checks `FileAttributes.ReparsePoint` on every enumerated child and skips symlinks. Directory symlinks are skipped without recursing (prevents infinite recursion on ancestor loops like `sub/parent_loop -> ..`) but still recorded in `ListedDirectories` / `FullyScannedDirectories` so stale entries from earlier symlink-following runs get purged. File symlinks are skipped entirely (prevents indexing the same content under two paths). Dangling symlinks (target missing) throw `FileNotFoundException` / `DirectoryNotFoundException` from `File.GetAttributes` and are also treated as reparse points so the scan does not abort. The project root itself is unaffected — when the user explicitly points `cdidx index` at a symlinked directory, the OS follows the symlink transparently during enumeration, because `_projectRoot` is the enumeration base, not a child path. There is no opt-in flag to follow symlinks; this matches `find` / `git ls-files` / `ripgrep` behavior without `-L`.
...
- **シンボリックリンクはスキャン時にスキップする** — `FileIndexer.EnumerateDirectory` は列挙した子エントリごとに `FileAttributes.ReparsePoint` を確認し、シンボリックリンクをスキップする。ディレクトリシンボリックリンクは再帰せずスキップするため、`sub/parent_loop -> ..` のような祖先ループでの無限再帰を防げる。同時に `ListedDirectories` / `FullyScannedDirectories` には登録しておき、過去にリンクを辿って取り込まれていた古いエントリを purge できるようにする。ファイルシンボリックリンクは完全にスキップし、同じ内容を二重にインデックスしない。ターゲットが存在しない dangling symlink は `File.GetAttributes` で `FileNotFoundException` / `DirectoryNotFoundException` になるため、それも reparse point 扱いとしてスキャンを中断させずに進める。プロジェクトルート自体は影響を受けない — ユーザーが `cdidx index` にシンボリックリンクされたディレクトリを明示指定した場合は、`_projectRoot` が列挙起点になるので OS が透過的にリンクを解決する。シンボリックリンクを辿るオプトインフラグはなく、`find` / `git ls-files` / `ripgrep` の `-L` 無指定時と同じ挙動に合わせる。
