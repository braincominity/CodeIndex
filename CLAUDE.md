...
src/CodeIndex/
  ...
  Indexer/SymbolExtractor.cs — Hybrid symbol extraction: compiled regexes for most languages, plus a lightweight JS/TS lexer/state machine for class-body methods, export-surface extraction (including multiline/type-only/import-attribute star/named re-exports and parenthesized or multiline/constrained TS-generic-arrow CommonJS bindings), scope filtering, and range resolution, and a dedicated HTML tag-structure state machine that walks tag openers, quoted/unquoted (including multi-line) attribute values, and masks `<script>`/`<style>`/`<textarea>`/`<title>` bodies plus `<!-- ... -->` comments
  Indexer/ReferenceExtractor.cs — Regex-based reference extraction with type-position `type_reference` edges and SQL source-list handling for comma-separated `FROM` clauses (31 languages with graph queries)
...
- **Language-aware reference extraction** — `references`, `callers`, `callees`, and `impact` are backed by an indexed reference table built only for languages where regex-based call/reference extraction is meaningful. Unsupported languages are expected to use `search` instead of receiving low-confidence pseudo-graph results. When a language is removed from graph support, `PurgeUnsupportedReferences` deletes its stale `symbol_references` rows on the next indexing run, and the read path additionally filters by supported languages to prevent stale edges from surviving between index runs. Nested generic constructor calls such as `new Dictionary<string, List<int>>()` (C#/Java) and C# generic method calls such as `Helper.DoWork<List<int>>()` are recovered by a depth-aware fallback scanner so the outer target still lands in the graph even though the flat regex fast-path cannot balance `>>`. JavaScript / TypeScript zero-argument constructor calls that legally omit `()` — for example `new Foo;`, `new Date;`, qualified targets like `new Demo.Provider;`, and one-level generic TypeScript forms like `new Box<number>;` — are emitted as `instantiate` edges via a dedicated language-gated path, while next-line `.bar()` / `[0]` continuations are suppressed so a line-ended `new Foo` does not become a phantom standalone instantiation. C# `: this(...)` / `: base(...)` initializers and Java constructor-body `this(...)` / `super(...)` are rewritten to point at the real target constructor (enclosing class/record for `this`, base type parsed from the class signature for `base`/`super`) so chain callers show up in `callers`, `references`, and `impact` instead of being silently dropped by the generic keyword-call filter. C#/Java type-position identifiers such as base lists, declaration types, generic constraints, `throws`, `is`/`as`/`instanceof`, and declaration-attached C# XML-doc `cref` entries from both `///` and `/** ... */` comments are indexed as `type_reference` rows so `references` / `impact` can see compile-time dependency edges without polluting the default dynamic call graph exposed by `callers` / `callees`; ordinary `//` / `////` comments, non-documenting block comments, multiline raw/verbatim string content that merely starts with `/**`, same-line non-doc text after `*/`, method-body XML-doc comments that merely precede a later declaration, brace-free field/property initializer continuations, brace-free expression lambdas, intervening top-level executable statements, same-line non-target code after `*/`, and other nested executable continuations stay excluded, while real declarations that begin later on the same physical line after the closing `*/` still inherit the doc comment only when no unrelated same-line code or declaration intervenes, and the emitted reference column stays anchored to the original physical source line. Consistent with that split, the CLI / MCP boundary treats `type_reference` as a `references --kind` value only: `references --kind type_reference` succeeds (no `unknown reference kind` warning), while `callers --kind type_reference` / `callees --kind type_reference` return `CommandExitCodes.UsageError` with a hint redirecting to `references --kind type_reference` (issue #444). Metadata kinds (`attribute` / `annotation`) follow the same split (issue #293). SQL read paths additionally resolve schema-qualified reference identities from each row's stored source-line context, recorded call column, and enclosing container before comparing them to definitions, so qualified `references` / `callers` / `impact` stay schema-scoped even when one line contains multiple qualified calls or the lookup is non-exact. Those graph/dependency readers fall back to the leaf identifier only for genuinely unqualified SQL source sites, keeping `deps`, `unused`, and `hotspots` aligned with qualified calls without regressing bare-call support or double-counting `EXEC dbo.fn_Target; EXEC sales.fn_Target;`. Once a row already has a recorded call column, those downstream readers no longer whole-line-upgrade that row to a later qualified token, so trailing comments, string literals, or a second qualified call cannot steal the earlier unqualified edge. Qualified SQL `callees` queries likewise keep leaf fallback disabled unless the caller query itself is unqualified, so `callees sales.Caller` no longer widens to `dbo.Caller`. SQL extractors also accept optional whitespace around qualified-name dots, so definitions/calls such as `[sales] . [fn_Target]` and `[dbo] . [fn_Target]` keep their full qualified identity instead of truncating at the first segment. The same SQL no-parens extractor now preserves ANSI / PostgreSQL double-quoted call targets such as `CALL "sales"."proc_name"` and `EXEC "dbo"."fn_Target"` instead of stripping them as string literals, while true single-quoted SQL string literals remain masked. Definition-oriented readers also canonicalize quoted qualified SQL names (`[dbo].[fn_X]` → `dbo.fn_X`) before matching, and they only fall back to the leaf identifier for unqualified queries so exact qualified lookups do not widen to sibling schemas that merely share the same leaf name. Exact SQL definition matching also preserves segment count, so a quoted single identifier that contains a dot (`"sales.fn_Target"`) does not collide with a real qualified name (`sales.fn_Target`). SQL exact graph leaf fallback also stays on the Unicode folded exact path, and both quoted qualified and unqualified Unicode exact definition lookups now use the folded normalized path, so queries such as `dbo.Äpfel` / `dbo.äpfel` and bare `Äpfel` / `äpfel` keep matching both leaf call rows and stored definitions such as `[dbo].[Äpfel]` or `dbo.Äpfel` instead of silently degrading to ASCII-only `NOCASE`. Exact multi-name SQL `symbols --count` lookups also bind folded leaf parameters on that same `_foldReady` path, so Unicode leaf query sets no longer fail with missing-parameter database errors.
  Exact SQL graph/dependency readers also preserve the resolved segment count, so a quoted single identifier containing a dot such as `"sales.fn_Target"` stays distinct from the real qualified name `sales.fn_Target` across exact `references` / `callers` / `impact` and aggregate `deps` / `unused` / `hotspots`.
...
- **Symlinks are skipped during scan** — The reparse-point check lives in `FileIndexer.GetFileIndexability`, so every caller gets the same skip behavior: the full scan, the `--files` / `--commits` / dry-run update paths in `IndexCommandRunner`, and in particular the Windows path (which previously returned `Supported` unconditionally and let symlinks through). `File.GetAttributes` uses `lstat`-equivalent semantics on both Windows and Unix in .NET, which is what we need — the Unix `stat()` used by `UnixFileStatus.TryGetFileMode` follows symlinks, so without the reparse-point guard a symlink-to-regular-file would otherwise pass the `RegularFile` check in update mode. Directory symlinks are skipped without recursing (prevents infinite recursion on ancestor loops like `sub/parent_loop -> ..`) and recorded in both `ListedDirectories` / `FullyScannedDirectories` (for the existing immediate-parent purge path) AND a new `SymlinkPrunedDirectories` set consumed by `DbWriter.PurgeFilesOutsideRetainedSetWithinListedDirectories`. The writer walks `LastIndexOf('/')` ancestors so it can authoritatively purge the **entire subtree** under a symlink (e.g. `sub/parent_loop/nested/deep.py` left behind by an earlier symlink-following run) while refusing to match a sibling directory that merely shares a name prefix (e.g. `sub/parent_loop_sibling/keep.py` is not touched by a prune prefix of `sub/parent_loop`). File symlinks are skipped entirely (prevents indexing the same content under two paths). Dangling symlinks (target missing) throw `FileNotFoundException` / `DirectoryNotFoundException` from `File.GetAttributes` and are also treated as reparse points so the scan does not abort. The project root itself is unaffected — when the user explicitly points `cdidx index` at a symlinked directory, the OS follows the symlink transparently during enumeration, because `_projectRoot` is the enumeration base, not a child path. There is no opt-in flag to follow symlinks; this matches `find` / `git ls-files` / `ripgrep` behavior without `-L`.
- **`.cdidx/` directory** — By default, `cdidx index` stores index files in `<projectPath>/.cdidx/codeindex.db` (not the caller's cwd). The directory is auto-created on first `cdidx index` and auto-added to `.git/info/exclude` so users don't touch `.gitignore`. User-authored `.gitignore` rules are also honored during scanning, and `.cdidxignore` can be used for cdidx-only exclusions. In a git worktree, `.git` is a file (not a directory), so `GitHelper.ResolveGitCommonDir()` follows the chain to find the shared `.git/` where `info/exclude` lives. This is a standard Git mechanism (used by git-lfs, Husky, JetBrains IDEs, etc.).
...
### Claude Code guard policy
This repository currently uses a deny-oriented `bash-guard.py` plus a minimal tracked `.claude/settings.json` so common work does not stall on repetitive confirmation dialogs. In normal operation, `dotnet`, `git` (except force push), `gh`, `codex exec`, `/tmp`-scoped file operations, and read-oriented shell commands such as `cat`, `wc`, `tail`, `head`, `awk`, `sed`, and `diff` are expected to pass without prompt churn, while dangerous commands remain blocked. Hard bans include command substitution (`$(...)`), `rm -rf`, network transfer commands such as `curl`, `wget`, `ssh`, and `scp`, force push, `codex:rescue`, and direct `python` / `python3` / `perl` / `node` execution. `node` is only acceptable for the reviewed `codex-companion.mjs` path already covered by the guard; Python remains prohibited as a shell execution path even for local helper scripts.

`gh` access is intentionally configured around read-only use of `~/.config/gh/**`. Claude Code may read existing GitHub CLI auth/config state from that directory, but it must not mutate it. Treat tracked `.claude/settings.json` and `.claude/hooks/bash-guard.py` as policy files: do not edit them during routine implementation work unless the task is explicitly about changing Claude Code guard behavior.

For this repository, using `dangerouslyDisableSandbox: true` is acceptable only for `dotnet test` when the sandbox itself is the blocker. Do not use it for `dotnet build`, `dotnet run`, `gh`, `git`, `codex exec`, Python, or general shell convenience. If `dotnet test` fails specifically because of sandbox restrictions and there is no equivalent sandboxed path, prefer that targeted exception over broadening the policy elsewhere.
...
src/CodeIndex/
  ...
  Indexer/SymbolExtractor.cs — ハイブリッドなシンボル抽出（大半はコンパイル済み正規表現、JS/TS は class body method・export surface 抽出（multiline star/named re-export や parenthesized CommonJS binding を含む）・scope filtering・range 解決向けの軽量 lexer / state machine を追加、HTML は汎用パターンループではなくタグ構造を理解した文字単位 state machine でタグ開始・引用符付き/なし（複数行含む）属性値・`<script>` / `<style>` / `<textarea>` / `<title>` 本体・`<!-- ... -->` コメントをマスクして属性名に似た文字列から phantom を防ぐ）
...
- **言語差分を考慮した参照抽出** — `references`、`callers`、`callees`、`impact` は、正規表現ベースの call/reference 抽出が意味を持つ言語だけに対して構築する参照テーブルに支えられる。未対応言語には低信頼な疑似グラフ結果を返さず、`search` を使う前提にする。graph 対応から外れた言語は、次回インデックス時に `PurgeUnsupportedReferences` が古い `symbol_references` 行を削除し、read path 側も対応言語だけに絞って stale edge の残留を防ぐ。`new Dictionary<string, List<int>>()` のような C#/Java の nested generic コンストラクタ呼び出しや、`Helper.DoWork<List<int>>()` のような C# generic method call は、平坦な regex fast-path で `>>` を釣り合わせられなくても depth-aware fallback scanner で拾い直し、外側 target を graph に残す。C# の `: this(...)` / `: base(...)` イニシャライザと、Java のコンストラクタ本体冒頭文 `this(...)` / `super(...)` は、汎用キーワード除外で無言のまま落とさず、呼び先を実体コンストラクタに書き換える（`this` は外側の class/record、`base` / `super` は外側クラスのシグネチャから解析した基底型）ため、`callers`、`references`、`impact` で連鎖の呼び出し元が見えるようになる。さらに C#/Java の継承リスト、宣言型、generic 制約、`throws`、`is` / `as` / `instanceof`、後続宣言へ結び付く実際の C# XML doc `cref`（`///` と `/** ... */` の両方）などの型位置参照も `type_reference` 行として索引し、既定の `callers` / `callees` の動的 call graph を汚さずに `references` / `impact` へ compile-time dependency edge を届ける。通常の `//` / `////` コメント、通常の block comment、同一物理行の `*/` 後ろに続くテキストに加え、トップレベル実行文が doc comment と後続宣言の間に割り込むケースや、same-line `*/` 後ろに非対象 code / 宣言があるケースも doc-comment 経路から除外し、closing `*/` 後ろの declaration が有効なのは attributes だけを挟んで直後の実宣言へ続く場合に限る。出力する列位置も元の物理ソース行に固定する。CLI / MCP 境界もこの分離に合わせ、`type_reference` を `references --kind` 専用の値として扱う: `references --kind type_reference` は警告なしで成功し、`callers --kind type_reference` / `callees --kind type_reference` は `CommandExitCodes.UsageError` を返して `references --kind type_reference` に誘導する（issue #444）。metadata kinds (`attribute` / `annotation`) も同じ契約で分離する（issue #293）。SQL の read path は、各 reference 行に保存した source-line context、記録済み call 列位置、enclosing container から schema-qualified な参照識別子を復元して定義と照合するため、qualified な `references` / `callers` / `impact` が sibling schema へ広がらず、1 行に複数の qualified call があっても、また query が non-exact でも取り違えず、同一行の `CREATE ... AS EXEC ...` でも DDL header token だけを落として実際の call target は保持できる。これらの graph/dependency reader は、SQL 側の source site が本当に unqualified なときだけ leaf 識別子へフォールバックするので、qualified call を含む `deps`、`unused`、`hotspots` を bare-call 互換のまま schema 単位で揃えつつ、`EXEC dbo.fn_Target; EXEC sales.fn_Target;` のような同一行 multi-call も二重計上しない。さらに列位置が記録されている row は、その列に qualified token が無ければ whole-line の後続 qualified token へ昇格させないため、行末コメント・文字列リテラル・後続の別 call が先頭の unqualified edge を横取りできない。qualified な SQL `callees` query でも、caller query 自体が unqualified なとき以外は leaf fallback を無効化したため、`callees sales.Caller` が `dbo.Caller` へ広がらない。SQL extractor は qualified-name のドット前後に任意の空白も許容するため、`[sales] . [fn_Target]` や `[dbo] . [fn_Target]` のような定義・呼び出しも先頭セグメントで切れず完全な qualified identity を保つ。definition 系 reader も quoted qualified SQL name (`[dbo].[fn_X]` → `dbo.fn_X`) を正規化してから照合し、leaf 識別子へのフォールバックは unqualified query のときだけに限定することで、同じ leaf name を共有する sibling schema へ exact qualified lookup が広がらない。さらに exact SQL 定義照合では segment 数も保持するため、ドットを含む quoted single identifier (`"sales.fn_Target"`) が本物の qualified name (`sales.fn_Target`) と衝突しない。SQL の exact graph leaf fallback も Unicode folded exact path に揃え、quoted qualified な Unicode exact lookup でも folded normalized path を使うため、`dbo.Äpfel` / `dbo.äpfel` のような query でも leaf として索引された `äpfel` の call/reference 行と `[dbo].[Äpfel]` の保存済み定義を ASCII 専用 `NOCASE` に落とさず拾える。さらに exact multi-name SQL `symbols --count` も同じ `_foldReady` 経路で folded leaf parameter を束縛するため、Unicode leaf query の組み合わせが missing-parameter な database error に落ちない。
...
- **シンボリックリンクはスキャン時にスキップする** — reparse point の判定は `FileIndexer.GetFileIndexability` の先頭に置いているため、フルスキャンはもちろん、`IndexCommandRunner` の `--files` / `--commits` / dry-run といった update 経路や、以前は無条件に `Supported` を返して symlink を通していた Windows 経路も含め、全呼び出し元で同じ skip 挙動が効く。`File.GetAttributes` は .NET 上の Windows / Unix 双方で lstat 相当（symlink target を辿らない）なのでここで使う判定として正しい — `UnixFileStatus.TryGetFileMode` が呼ぶ `stat()` は symlink を辿るため、このガードが無いと symlink→通常ファイルが `RegularFile` 判定を素通りし update モードで読み込まれてしまう。ディレクトリシンボリックリンクは再帰せずスキップするため、`sub/parent_loop -> ..` のような祖先ループでの無限再帰を防げる。同時に `ListedDirectories` / `FullyScannedDirectories`（既存の immediate-parent purge 経路向け）に加えて、新設の `SymlinkPrunedDirectories` 集合にも登録する。`DbWriter.PurgeFilesOutsideRetainedSetWithinListedDirectories` はこの集合を使い、`LastIndexOf('/')` で祖先を辿って symlink ディレクトリ配下の **subtree 全体** を authoritative に purge する（過去の symlink 追従で残った `sub/parent_loop/nested/deep.py` のような深い子孫も対象）。同時に、`sub/parent_loop` という prune prefix が `sub/parent_loop_sibling/keep.py` のような名前 prefix だけが一致する兄弟ディレクトリを巻き込まないよう、ディレクトリ境界に揃った一致のみを拾う。ファイルシンボリックリンクは完全にスキップし、同じ内容を二重にインデックスしない。ターゲットが存在しない dangling symlink は `File.GetAttributes` で `FileNotFoundException` / `DirectoryNotFoundException` になるため、それも reparse point 扱いとしてスキャンを中断させずに進める。プロジェクトルート自体は影響を受けない — ユーザーが `cdidx index` にシンボリックリンクされたディレクトリを明示指定した場合は、`_projectRoot` が列挙起点になるので OS が透過的にリンクを解決する。シンボリックリンクを辿るオプトインフラグはなく、`find` / `git ls-files` / `ripgrep` の `-L` 無指定時と同じ挙動に合わせる。
...
### Claude Code ガード方針
このリポジトリでは現在、deny ベースの `bash-guard.py` と最小限の追跡対象 `.claude/settings.json` を組み合わせ、毎回の確認ダイアログで作業が止まらないことを優先しています。通常運用では `dotnet`、`git`（force push を除く）、`gh`、`codex exec`、`/tmp` 配下の操作、`cat`、`wc`、`tail`、`head`、`awk`、`sed`、`diff` などの読み取り寄りコマンドは不要な確認なしで通る前提です。一方で危険コマンドは止め続けます。厳格に禁止する対象は command substitution（`$(...)`）、`rm -rf`、`curl`、`wget`、`ssh`、`scp` などのネットワーク転送、force push、`codex:rescue`、および `python` / `python3` / `perl` / `node` の直接実行です。`node` は guard 済みの `codex-companion.mjs` に限って例外で、それ以外の Python 系実行経路はローカル補助スクリプト用途でも禁止です。

`gh` まわりは `~/.config/gh/**` の read-only 利用を前提にします。Claude Code はそのディレクトリから既存の GitHub CLI 認証・設定を読むことはできますが、書き換えてはいけません。追跡対象の `.claude/settings.json` と `.claude/hooks/bash-guard.py` はポリシーファイルとして扱い、Claude Code のガード挙動自体を変更するタスクでない限り、通常の実装作業で編集しません。

このリポジトリで `dangerouslyDisableSandbox: true` を使ってよいのは、sandbox 自体が原因で `dotnet test` が塞がれている場合だけです。`dotnet build`、`dotnet run`、`gh`、`git`、`codex exec`、Python 実行、一般的な shell の利便性目的には使ってはいけません。`dotnet test` が sandbox 制約に起因して失敗し、sandbox 内の等価手段がないときだけ、その限定的な例外を使い、他の経路まで緩めないでください。
