...
- **Literal-safe search by default** — Search queries are quoted token-by-token to avoid FTS syntax errors by default. Raw FTS5 syntax is opt-in via `--fts` or MCP `rawQuery`. Tokens that contain CJK script (Han, Hiragana, Katakana, Hangul, and halfwidth Katakana) are emitted as FTS5 prefix phrases (`"計算"*`) instead of bare phrases, so `search 計算` can still find `計算する` even though the default `unicode61` tokenizer treats the whole CJK run as a single token. Non-CJK tokens — including Latin-diacritic (`naïve`), Greek, Cyrillic, and emoji-mixed text — intentionally keep exact-phrase semantics: `unicode61` tokenizes Latin-diacritic normally, and promoting `foo🎉` to prefix would be parsed by FTS5 as `foo*` and over-match unrelated ASCII neighbors like `foobar`. Emoji substring search is a separate limitation — `unicode61` excludes symbol categories entirely, so `search 🎉` requires a tokenizer-level schema change (tracked via issue #198 follow-ups).
...
- **デフォルトはリテラル安全検索** — 検索クエリは既定ではトークンごとに引用し、FTS構文エラーを避ける。生のFTS5構文は `--fts` またはMCPの `rawQuery` で明示 opt-in。CJK スクリプト（漢字・ひらがな・カタカナ・ハングル・半角カナ）を含むトークンは、素のフレーズではなく FTS5 prefix phrase (`"計算"*`) として発行する。unicode61 既定トークナイザは CJK 連続を単一トークンとして扱うため、この prefix 昇格により `search 計算` が `計算する` を含むチャンクにマッチする。CJK 以外の非 ASCII トークン（`naïve` 等の Latin-diacritic、Greek、Cyrillic、絵文字混在）は意図的に完全一致のまま保つ。unicode61 は Latin-diacritic を通常どおりトークン化し、`foo🎉` を prefix 化してしまうと FTS5 側で `foo*` として解釈されて `foobar` など無関係な ASCII 近傍にまで拡張してしまうため。絵文字の部分一致は別問題で、`unicode61` が symbol カテゴリを丸ごと除外しているため `search 🎉` はトークナイザ段階で 0 件になる。これはスキーマレベルの変更が必要で、issue #198 の follow-up として扱う。
