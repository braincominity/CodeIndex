...
- **言語差分を考慮した参照抽出** — `references`、`callers`、`callees` は、正規表現ベースの call/reference 抽出が意味を持つ言語だけに対して構築する参照テーブルに支えられる。未対応言語には低信頼な疑似グラフ結果を返さず、`search` を使う前提にする。`new Dictionary<string, List<int>>()` のような C#/Java の nested generic コンストラクタ呼び出しや、`Helper.DoWork<List<int>>()` のような C# generic method call は、平坦な regex fast-path で `>>` を釣り合わせられなくても depth-aware fallback scanner で拾い直し、外側 target を graph に残す。C# の `: this(...)` / `: base(...)` イニシャライザと、Java のコンストラクタ本体冒頭文 `this(...)` / `super(...)` は、汎用キーワード除外で無言のまま落とさず、呼び先を実体コンストラクタに書き換える（`this` は外側の class/record、`base` / `super` は外側クラスのシグネチャから解析した基底型）ため、`callers`、`references`、`impact` で連鎖の呼び出し元が見えるようになる。さらに C#/Java の継承リスト、宣言型、generic 制約、`throws`、`is` / `as` / `instanceof`、実際の C# XML doc `///` `cref` などの型位置参照も `type_reference` 行として索引し、既定の `callers` / `callees` の動的 call graph を汚さずに `references` / `impact` へ compile-time dependency edge を届ける。SQL の read path は schema-qualified な symbol 名を leaf 識別子へも正規化して論理 graph/dependency lookup (`dbo.fn_X` ↔ `fn_X`) に使うため、qualified query、`deps` edge、unused 判定、hotspot 件数が bare な callee token で保存される SQL reference 行と揃う。definition 系 reader も quoted qualified SQL name (`[dbo].[fn_X]` → `dbo.fn_X`) を正規化してから照合し、leaf 識別子へのフォールバックは unqualified query のときだけに限定することで、同じ leaf name を共有する sibling schema へ exact qualified lookup が広がらない。さらに exact SQL 定義照合では segment 数も保持するため、ドットを含む quoted single identifier (`"sales.fn_Target"`) が本物の qualified name (`sales.fn_Target`) と衝突しない。SQL の exact graph leaf fallback も Unicode folded exact path に揃え、quoted qualified な Unicode exact lookup でも folded normalized path を使うため、`dbo.Äpfel` / `dbo.äpfel` のような query でも leaf として索引された `äpfel` の call/reference 行と `[dbo].[Äpfel]` の保存済み定義を ASCII 専用 `NOCASE` に落とさず拾える。さらに exact multi-name SQL `symbols --count` も同じ `_foldReady` 経路で folded leaf parameter を束縛するため、Unicode leaf query の組み合わせが missing-parameter な database error に落ちない。SQL の same-line definition suppression もこの leaf alias に対して列位置つきになっており、DDL header token だけを phantom self-call として落とし、real な same-line cross-schema call は保持する。
