...
- **Hybrid symbol extraction** — Intentionally simple and dependency-light. Most languages still use compiled regex patterns, while JavaScript/TypeScript add a lightweight lexer/state machine for class-body method extraction, scope filtering, synthetic class-expression binding detection, and JS/TS-specific range resolution where regex alone is too brittle. Accuracy is still secondary to speed and portability, but the index stores richer symbol metadata such as definition ranges, optional body ranges, signatures, enclosing symbols, visibility, and return types when the regex patterns or JS/TS state machine can infer them.
