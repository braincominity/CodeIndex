...
| Java | methods (including same-line leading annotations, same-line `@interface` members with later same-line siblings such as `value(); int age();`, and compact record constructors even when the record header contains annotation-array arguments or the constructor body opener moves to the next line), static final, enum members (body-scoped scanner that tracks strings/chars/comments/text blocks and stops at the first top-level `;`, so method calls like `\tRED();` outside the enum body are not captured; enum members with anonymous bodies keep body ranges so nested override/helper methods stay attached to the member body) | class, record, sealed, @interface | -- | interface | enum | record primary components | -- | import | yes |
...
| Rust | fn, macro_rules!, const, static | impl, type alias | struct, union | trait | enum | -- | -- | use | yes |
| Java | メソッド（同一行の先頭アノテーション付き宣言、`value(); int age();` のような同一行 `@interface` メンバーの後続 sibling、record header に annotation-array 引数を含むものや constructor body の opening brace が次行へ送られたものを含む compact record constructor を含む）, static final, enum メンバー（文字列・char・コメント・text block を追跡する body-scoped scanner で抽出し、最初の top-level `;` で停止するため、enum 本体外の `\tRED();` のようなメソッド呼び出しを誤検出しない。匿名メンバー本体を持つ enum メンバーは body range を保持し、その本体内の override / helper メソッドがメンバー本体配下にぶら下がる） | class, record, sealed, @interface | -- | interface | enum | record primary component | -- | import | yes |
