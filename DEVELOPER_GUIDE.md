...
SQL の graph 抽出は `FROM` / `JOIN` / `USING` / `INSERT INTO` / `UPDATE` / `TRUNCATE TABLE` / `MERGE INTO` のような名前付き source/target 形も `reference` edge として出力します。T-SQL の `#temp` / `##temp` は `INSERT INTO #temp`、`SELECT ... INTO #temp`、複数行 `SELECT ... INTO #temp`、`CREATE TABLE #temp` のような target 側で認識し、その temp object が先行 statement によって確立された後続 read、同一行の `;` 区切り statement と semicolon-less な後続 statement 境界を含む statement-order の後続 read でも `reference` 経路に残します。`UPDATE TOP (...)`、`MERGE TOP (...) INTO`、`FROM ONLY ...`、`FROM/JOIN LATERAL ...` のような SQL 専用 modifier は実 object 名を解決する前に読み飛ばします。`MERGE ... WHEN MATCHED THEN UPDATE SET ...` の action clause からは `SET` を target reference として出さず、`UPDATE [SET] SET ...` のような quoted target は正当な target として維持します。`EXEC` / `CALL` と `FROM dbo.fn_x(...)` / `FROM [dbo].[fn_x](...)` / ``FROM `fn_x`(...)`` / `FROM "fn_x"(...)` のような table-valued function 使用は引き続き `call` 経路に残し、ANSI の二重引用符付き source identifier（`"users"` / `"schema"."orders"` など）は `reference` 経路に残します。MySQL / MariaDB の `# comment` 末尾は graph 抽出前に除去し、単独の手続き系 `SELECT ... INTO variable` は object reference にしません。
