...
| `callers` / `callees` (デフォルト) | source ↔ container | `CallGraphReferenceKindsSql`。runtime call / event に加えて dependency-oriented な `capture`、`friend`、`consumes_hook`、`augmentation` を含む。metadata 種別は CLI / MCP `--kind` 境界で拒否 | `DbReader.GetCallers` / `DbReader.GetCallees` |
| `impact` callers mode | 推移的 forward (BFS) | `GetCallersExact` 経由で `capture` を含む `CallGraphReferenceKindsSql` | `DbReader.GetTransitiveCallers` |
