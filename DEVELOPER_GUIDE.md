...
SQL の graph 抽出は `FROM` / `JOIN` / `USING` / `INSERT INTO` / `UPDATE` / `TRUNCATE TABLE` / `MERGE INTO` のような名前付き source/target 形も `reference` edge として出力します。T-SQL の `#temp` / `##temp` は `INSERT INTO #temp` や `SELECT ... INTO #temp` のような target 側で認識し、その temp object が同一 SQL 本体内で後続 read される場合も `reference` 経路に残します。`EXEC` / `CALL` と `FROM dbo.fn_x(...)` / `FROM [dbo].[fn_x](...)` / ``FROM `fn_x`(...)`` のような table-valued function 使用は引き続き `call` 経路に残し、MySQL / MariaDB の `# comment` 末尾は graph 抽出前に除去します。単独の手続き系 `SELECT ... INTO variable` は object reference にしません。
