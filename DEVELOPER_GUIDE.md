...
SQL graph extraction also emits `reference` edges for named source/target forms such as `FROM`, `JOIN`, `USING`, `INSERT INTO`, `UPDATE`, `TRUNCATE TABLE`, and `MERGE INTO`. T-SQL `#temp` / `##temp` targets are recognized on write-side forms such as `INSERT INTO #temp` and `SELECT ... INTO #temp`, and later same-body reads of those temp objects also stay on the `reference` path. `EXEC` / `CALL` sites plus table-valued-function uses like `FROM dbo.fn_x(...)`, `FROM [dbo].[fn_x](...)`, and ``FROM `fn_x`(...)`` stay on the `call` path, while MySQL / MariaDB `# comment` tails are still stripped before graph extraction. Standalone procedural `SELECT ... INTO variable` does not become an object reference.
