...
SQL graph extraction also emits `reference` edges for named source/target forms such as `FROM`, `JOIN`, `USING`, `INSERT INTO`, `UPDATE`, `TRUNCATE TABLE`, and `MERGE INTO`, treats `SELECT ... INTO #temp` / `SELECT ... INTO ##temp` as temp-table references, and keeps `EXEC` / `CALL` sites plus table-valued-function uses like `FROM dbo.fn_x(...)` on the `call` path. Standalone procedural `SELECT ... INTO variable` does not become an object reference.
...
SQL の graph 抽出は `FROM` / `JOIN` / `USING` / `INSERT INTO` / `UPDATE` / `TRUNCATE TABLE` / `MERGE INTO` のような名前付き source/target 形も `reference` edge として出力し、`SELECT ... INTO #temp` / `SELECT ... INTO ##temp` も temp-table 参照として扱います。`EXEC` / `CALL` と `FROM dbo.fn_x(...)` のような table-valued function 使用は引き続き `call` 経路に残し、単独の手続き系 `SELECT ... INTO variable` は object reference にしません。
