...
The repository root `nuget.config` is part of this supply-chain boundary. It clears machine-wide package sources, allows only `https://api.nuget.org/v3/index.json`, maps every package ID to that source, and requires signed packages. The trusted signer is the current NuGet.org repository-signing certificate, so restore accepts packages countersigned by nuget.org and rejects unsigned packages or packages from unconfigured feeds. When NuGet.org rotates its repository-signing certificate, update `nuget.config` in the same change as the restore validation.
...
リポジトリルートの `nuget.config` もこの supply-chain 境界の一部です。machine-wide な package source を消去し、`https://api.nuget.org/v3/index.json` だけを許可し、すべての package ID をその source に map し、署名付き package を必須にします。trusted signer は現在の NuGet.org repository-signing certificate で、nuget.org に countersign された package だけを restore で受け入れ、未署名 package や未設定 feed 由来の package は拒否します。NuGet.org が repository-signing certificate を rotate した場合は、restore 検証と同じ変更で `nuget.config` を更新してください。
