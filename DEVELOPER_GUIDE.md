...
src/CodeIndex/
  ...
  Indexer/
    ...
    ReferenceExtractor.cs     — Regex-based call/reference extraction with a depth-aware fallback for nested-generic call sites (31 languages with graph queries)
  Mcp/
...
SQL graph extraction also emits `reference` edges for named source/target forms such as `FROM`, `JOIN`, `INSERT INTO`, `UPDATE`, `TRUNCATE TABLE`, `DELETE FROM`, `MERGE INTO`, and `MERGE ... USING`. T-SQL `#temp` / `##temp` targets are recognized on write-side forms such as `INSERT INTO #temp`, `UPDATE #temp`, `DELETE FROM #temp`, `SELECT ... INTO #temp`, multiline `SELECT ... INTO #temp`, and `CREATE TABLE #temp`; later statement-order reads of temp objects established by those earlier statements, including later same-line `;`-separated statements and later semicolon-less statement boundaries, also stay on the `reference` path. SQL-specific source/target modifiers such as `UPDATE TOP (...)`, `DELETE TOP (...) FROM`, `MERGE TOP (...) INTO`, `UPDATE ONLY ...`, `FROM ONLY ...`, and `FROM/JOIN LATERAL ...` are skipped before resolving the real object name. Bare `USING` is intentionally not treated as a generic source introducer because other SQL dialects reuse it for non-source clauses such as `CREATE INDEX ... USING btree (...)` and `ALTER TABLE ... USING lower(...)`. SQL Server target hints between `MERGE INTO <target>` and `USING <source>` are also allowed, so forms such as `MERGE INTO audit_log WITH (INDEX(ix), HOLDLOCK) AS t USING staging_log ...` keep the real source-side `reference`. `MERGE ... WHEN MATCHED THEN UPDATE SET ...` no longer emits the action-clause keyword `SET` as a table target, while quoted forms such as `UPDATE [SET] SET ...` remain valid. `EXEC` / `CALL` sites plus table-valued-function uses like `FROM dbo.fn_x(...)`, `FROM [dbo].[fn_x](...)`, ``FROM `fn_x`(...)``, and `FROM "fn_x"(...)` stay on the `call` path, while ANSI double-quoted source identifiers such as `"users"` / `"schema"."orders"` stay on the `reference` path. Double-quoted SQL text only participates in object extraction when the introducer keyword sits outside the quoted region, so dynamic SQL assignments such as `SET @sql = "SELECT * FROM users";` do not leak phantom `reference users` rows or falsely establish later `#temp` reads. Multiline `/* ... */` comments and PostgreSQL dollar-quoted bodies (`$$...$$` / `$tag$...$tag$`) are blanked before source/target scanning as well, so non-code SQL text cannot leak authoritative `reference` rows; same-line `$$...$$; SELECT ...; $$...$$` shapes close at the first real delimiter so later same-line statements are still visible. MySQL / MariaDB backslash-escaped single quotes inside `'...'` strings remain opaque to the source scanner, `# comment` tails are still stripped before graph extraction, and standalone procedural `SELECT ... INTO variable` does not become an object reference.
...
src/CodeIndex/
  ...
  Indexer/
    ...
    ReferenceExtractor.cs     — 対応言語向けの正規表現ベース参照抽出（nested generic 呼び出しは depth-aware fallback あり）
  Mcp/
...
C# の `Graph = yes` は callable/reference extraction と event subscription を指します。`new Dictionary<string, List<int>>()` のような nested generic コンストラクタ呼び出しや `Helper.DoWork<List<int>>()` のような C# generic method call も depth-aware fallback を通して同じ graph 経路に乗るため、外側の callee/constructor target が `references` / `callers` / `callees` / `impact` に残ります。`@class` / `@return` / `@if` のような verbatim identifier も C# の宣言名・call/instantiate 経路で受理され、保存される symbol/reference 名は qualified な namespace/import/container path（`Outer.@class` → `Outer.class`）や conversion operator の対象型（`implicit operator List<@class>` → `implicit operator List<class>`）を含めて先頭 `@` を外した意味上の識別子へ正規化されるため、lookup と graph matching は綴りの表記差分に依存しません。`Nested.A` や `Outer.First.None` のような qualified enum-member access も索引され、参照行は symbol extractor が復元できる最も内側の owner（クラス全体ではなく property / method など）を引き継ぎます。一方で `unused` は別の制約がまだ残っており、C# enum member を含む scope はその member が `unused` から除外されるため degraded として扱われ、enum declaration も偽陽性になりうるものとして残ります。

SQL は `CREATE SCHEMA` から `namespace` シンボルも出力しますが、上の要約表には namespace 専用の列がありません。
SQL の graph 抽出は `FROM` / `JOIN` / `INSERT INTO` / `UPDATE` / `TRUNCATE TABLE` / `DELETE FROM` / `MERGE INTO` / `MERGE ... USING` のような名前付き source/target 形も `reference` edge として出力します。T-SQL の `#temp` / `##temp` は `INSERT INTO #temp`、`UPDATE #temp`、`DELETE FROM #temp`、`SELECT ... INTO #temp`、複数行 `SELECT ... INTO #temp`、`CREATE TABLE #temp` のような target 側で認識し、その temp object が先行 statement によって確立された後続 read、同一行の `;` 区切り statement と semicolon-less な後続 statement 境界を含む statement-order の後続 read でも `reference` 経路に残します。`UPDATE TOP (...)`、`DELETE TOP (...) FROM`、`MERGE TOP (...) INTO`、`UPDATE ONLY ...`、`FROM ONLY ...`、`FROM/JOIN LATERAL ...` のような SQL 専用 modifier は実 object 名を解決する前に読み飛ばします。bare な `USING` は `CREATE INDEX ... USING btree (...)` や `ALTER TABLE ... USING lower(...)` のような非 source 句にも再利用されるため、汎用 source 導入子としては扱いません。さらに SQL Server で `MERGE INTO <target>` と `USING <source>` の間に入る target hint も許容するため、`MERGE INTO audit_log WITH (INDEX(ix), HOLDLOCK) AS t USING staging_log ...` のような形でも source 側の `reference` を維持します。`MERGE ... WHEN MATCHED THEN UPDATE SET ...` の action clause からは `SET` を target reference として出さず、`UPDATE [SET] SET ...` のような quoted target は正当な target として維持します。`EXEC` / `CALL` と `FROM dbo.fn_x(...)` / `FROM [dbo].[fn_x](...)` / ``FROM `fn_x`(...)`` / `FROM "fn_x"(...)` のような table-valued function 使用は引き続き `call` 経路に残し、ANSI の二重引用符付き source identifier（`"users"` / `"schema"."orders"` など）は `reference` 経路に残します。二重引用符付き SQL テキストは導入キーワード自体が引用内にある限り object 抽出の対象にしないため、`SET @sql = \"SELECT * FROM users\";` のような dynamic SQL 文字列から phantom `reference users` も、未実行の `SELECT ... INTO #temp` による phantom temp establishment も漏れません。さらに複数行 `/* ... */` コメントと PostgreSQL の dollar-quoted body（`$$...$$` / `$tag$...$tag$`）も source/target 走査前に空白化するため、非コード SQL テキストから authoritative な `reference` row は漏れません。同一行の `$$...$$; SELECT ...; $$...$$` 形では最初の実 close delimiter で body を閉じるため、後続の same-line statement も見えるままです。MySQL / MariaDB の `'...'` 文字列内にある backslash escape 付き single quote も source scanner には不透明のまま保ち、`# comment` 末尾は graph 抽出前に除去し、単独の手続き系 `SELECT ... INTO variable` は object reference にしません。
