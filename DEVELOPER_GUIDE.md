...
| C# | methods, ctors, explicit-interface impls, and indexers (including `ref` / `ref readonly` returns; guards named-argument labels only before qualified call expressions; allows `global::` / alias-qualified return types and spaced generic type tokens), operators, const, static readonly, enum members, #region, finalizers | class, record | struct, record struct, ref struct | interface | enum | property, expression-bodied, `ref` / `ref readonly` properties, record primary components | event, delegate (including spaced generic type tokens) | using, using alias | yes |
