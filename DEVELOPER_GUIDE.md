...
|---|---|---|---|---|---|---|---|---|:---:|
| Python | def, async def | class | -- | -- | -- | @property | -- | from/import | yes |
| JavaScript | function, トップレベル generator / async generator（`function*`、`async function*`）, アロー, メソッド（同一行の keyword / modifier 名、default 引数、computed、`#private`、generator、`async *`、`handleClick = () => {}` のような class-field アロープロパティを含む）, オブジェクトリテラルの method / accessor / generator ショートハンド（`{ foo() {}, get foo() {}, set foo(v) {}, *bar() {}, async baz() {} }`、`module.exports = { ... }` も含む）, `module.exports.foo = function () {}` / `module.exports.bar = () => {}` のような直接的な CommonJS named export binding | class, export default class, 同一行 sibling / statement-prefixed public class, クラス式, 複数行 / parenthesized / CommonJS クラス export | -- | -- | -- | `export * from` / `export { foo as bar } from` のような ESM re-export と、`module.exports = { foo: inner }` のような exported object-literal alias property を property 的な export surface symbol として抽出 | -- | import...from | yes |
| TypeScript | function, トップレベル generator / async generator（`function*`、`async function*`）, アロー, メソッド（generic / 同一行 object-return / conditional / function-return、default 引数、computed、`#private`、generator、`async *`、`handleClick = (): void => {}` のような戻り値型付き class-field アロープロパティを含む）, オブジェクトリテラルの method / accessor / generator ショートハンド（`{ foo(): T {}, get foo(): T {}, set foo(v: T) {}, *bar(): Generator<T> {} }`）, `module.exports.foo = function () {}` / `module.exports.bar = () => {}` のような直接的な CommonJS named export binding | class, export default class, 匿名 default `abstract class`, `export = class`, 同一行 sibling / statement-prefixed public class, 複数行 / parenthesized クラス式, type | -- | interface | enum, const enum | `export * from` / `export { foo as bar } from` / `export type { User } from` のような ESM/TS re-export と、`module.exports = { foo: inner }` のような exported object-literal alias property を property 的な export surface symbol として抽出 | -- | import...from | yes |
