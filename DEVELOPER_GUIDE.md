...
SQL graph extraction also emits `reference` edges for named source/target forms such as `FROM`, `JOIN`, `USING`, `INTO`, `UPDATE`, `TRUNCATE TABLE`, and `MERGE INTO`, while `EXEC` / `CALL` sites and table-valued-function uses like `FROM dbo.fn_x(...)` stay on the `call` path.
...
SQL の graph 抽出は `FROM` / `JOIN` / `USING` / `INTO` / `UPDATE` / `TRUNCATE TABLE` / `MERGE INTO` のような名前付き source/target 形も `reference` edge として出力し、`EXEC` / `CALL` と `FROM dbo.fn_x(...)` のような table-valued function 使用は引き続き `call` 経路に残します。
