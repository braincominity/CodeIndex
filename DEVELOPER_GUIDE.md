...
enforcing strict UTF-8 on input (BOM detection remains enabled, output is
BOM-less UTF-8, 64 KiB buffer, `AutoFlush = true`). Malformed UTF-8 bytes
...
`StdioMcpTransport` は #1558 以前と同じ stdio framing を維持しつつ、入力を strict UTF-8 として検証する（入力 BOM 検出は維持、出力は BOM なし UTF-8、64 KiB バッファ、`AutoFlush = true`）。不正な UTF-8 バイトは transport decode failure として表面化し、MCP ループはバイトを U+FFFD に黙って置換せず、invalid UTF-8 のヒント付き JSON-RPC `-32700` に変換する。
