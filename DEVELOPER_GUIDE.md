...
SQL の graph 抽出は `FROM` / `JOIN` / `USING` / `INSERT INTO` / `UPDATE` / `TRUNCATE TABLE` / `DELETE FROM` / `MERGE INTO` のような名前付き source/target 形も `reference` edge として出力します。T-SQL の `#temp` / `##temp` は `INSERT INTO #temp`、`UPDATE #temp`、`DELETE FROM #temp`、`SELECT ... INTO #temp`、複数行 `SELECT ... INTO #temp`、`CREATE TABLE #temp` のような target 側で認識し、その temp object が先行 statement によって確立された後続 read、同一行の `;` 区切り statement と semicolon-less な後続 statement 境界を含む statement-order の後続 read でも `reference` 経路に残します。`UPDATE TOP (...)`、`DELETE TOP (...) FROM`、`MERGE TOP (...) INTO`、`UPDATE ONLY ...`、`FROM ONLY ...`、`FROM/JOIN LATERAL ...` のような SQL 専用 modifier は実 object 名を解決する前に読み飛ばします。`MERGE ... WHEN MATCHED THEN UPDATE SET ...` の action clause からは `SET` を target reference として出さず、`UPDATE [SET] SET ...` のような quoted target は正当な target として維持します。`EXEC` / `CALL` と `FROM dbo.fn_x(...)` / `FROM [dbo].[fn_x](...)` / ``FROM `fn_x`(...)`` / `FROM "fn_x"(...)` のような table-valued function 使用は引き続き `call` 経路に残し、ANSI の二重引用符付き source identifier（`"users"` / `"schema"."orders"` など）は `reference` 経路に残します。二重引用符付き SQL テキストは導入キーワード自体が引用内にある限り object 抽出の対象にしないため、`SET @sql = \"SELECT * FROM users\";` のような dynamic SQL 文字列から phantom `reference users` は漏れません。さらに複数行 `/* ... */` コメントと PostgreSQL の dollar-quoted body（`$$...$$` / `$tag$...$tag$`）も source/target 走査前に空白化するため、非コード SQL テキストから authoritative な `reference` row は漏れません。MySQL / MariaDB の `# comment` 末尾は graph 抽出前に除去し、単独の手続き系 `SELECT ... INTO variable` は object reference にしません。
