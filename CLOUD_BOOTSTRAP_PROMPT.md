...
Run the first command when `version.json` should select the release, the second
when you need to probe a specific release, and the third when probing through a
mirror/proxy endpoint.
...
```bash
/root/.local/bin/cdidx --version                                     # must print a real version, v0.0.0 is NG
/root/.local/bin/cdidx . --db /tmp/smoke.db                          # SQLite write path (human output)
/root/.local/bin/cdidx status --db /tmp/smoke.db                     # SQLite read path
...
最初のコマンドは `version.json` のバージョンで probe する場合、2 つ目は
特定リリースを probe する場合、3 つ目は mirror/proxy 経由で probe する場合に
使います。
...
```bash
/root/.local/bin/cdidx --version                                     # 実バージョンが出ること。v0.0.0 は NG
/root/.local/bin/cdidx . --db /tmp/smoke.db                          # SQLite 書き込み経路（人間向け出力）
/root/.local/bin/cdidx status --db /tmp/smoke.db                     # SQLite 読み取り経路
/root/.local/bin/cdidx search "install" --path install.sh --snippet-lines 4 --db /tmp/smoke.db
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}' | /root/.local/bin/cdidx mcp
```
