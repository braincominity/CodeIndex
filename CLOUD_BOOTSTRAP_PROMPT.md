...
now has two intentionally narrow bootstrap exceptions:
...
installing in Codex, use the fully expanded absolute path from Step 1.5 below
(or the `CDIDX` variable after resolving that absolute path) for every
`cdidx` command. If the exact installer command is still blocked, the Codex
session is running an older checkout of this repo's guard; update to a branch
that contains this bootstrap exception before trying to install again. This
guard change does not bypass upstream egress policy: a real `CONNECT tunnel
failed, response 403` still requires an allow-listed artifact host path or a
reachable mirror/proxy endpoint.
...
を読み込みます。この guard には、次の 2 種類だけを通す意図的に狭い
bootstrap 例外を入れています:
...
Codex でインストールした後は、次の Step 1.5 で得る完全展開済み絶対パス
（またはその絶対パスを入れた `CDIDX` 変数）で毎回 `cdidx` を呼んでください。
正確な installer コマンドまでブロックされる場合、その Codex セッションは
この bootstrap 例外を含まない古い checkout の guard を実行しています。
インストールを再試行する前に、この例外を含むブランチへ更新してください。
この guard 変更は upstream egress policy を迂回するものではありません:
本物の `CONNECT tunnel failed, response 403` は、少なくとも 1 つの artifact
配信経路の allow-list、または実際に到達可能な mirror/proxy endpoint が
必要です。
